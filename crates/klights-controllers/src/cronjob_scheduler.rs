...
pub fn compute_next_fire(cj: &Value, now: DateTime<Utc>) -> Result<Option<DateTime<Utc>>> {
    ...
    let schedule = match crate::cronjob::parse_cron_schedule(schedule_str) {
        Ok(s) => s,
        Err(_) => return Ok(None),
    };

    if let Some(due) = crate::cronjob::most_recent_cronjob_schedule_time(cj, now, &schedule, true)?
    {
        return Ok(Some(if due <= now { now } else { due }));
    }

    let reference =
        match crate::cronjob::most_recent_cronjob_schedule_time(cj, now, &schedule, false)? {
            Some(most_recent_without_deadline) => most_recent_without_deadline,
            None => crate::cronjob::cronjob_schedule_lower_bound(cj, now, false),
        };
