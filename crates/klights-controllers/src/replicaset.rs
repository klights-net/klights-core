...
#[async_trait]
pub trait ReplicaSetStore: crate::gc::GcResourceStore + Send + Sync {
...
pub async fn reconcile_replicaset(
    ...
    let reconcile_lock = coordination.reconcile_lock(
        crate::CoordinatedControllerKind::ReplicaSet,
        namespace,
        name,
    );
    ...
    let live_replicaset = crate::resource_projection::with_resource_version(
        live_resource.data,
    ...
    for pod in rs_owned {
        if !pod_matches_selector_or_pending_hash_stamp(&pod.data, selector, owned_by_deployment) {
            ...
            if crate::common::remove_owner_reference_by_uid(&mut released_pod, "ReplicaSet", uid) {
    ...
    if owned_pods.len() < replicas {
        ...
        for pod in all_pods {
                ...
                && pod_is_active(&pod.data)
            {
                ...
                crate::common::append_owner_reference(
                    &mut adopted_pod,
