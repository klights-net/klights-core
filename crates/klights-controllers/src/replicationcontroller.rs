...
#[async_trait]
pub trait ReplicationControllerStore: crate::gc::GcResourceStore + Send + Sync {
...
pub async fn reconcile_replicationcontroller(
    ...
    rc: &Value,
    reconcile_context: crate::ControllerReconcileContext<'_>,
) -> Result<()> {
    ...
    let reconcile_lock = coordination.reconcile_lock(
        crate::CoordinatedControllerKind::ReplicationController,
        namespace,
        rc_name,
    );
    ...
    let rc = crate::resource_projection::with_resource_version(
        live_resource.data,
...
fn resource_quota_constrains_pod_creates(quota: &Value, pod: &Value) -> bool {
    ...
    if !scopes.is_empty() && !crate::resource_quota::pod_matches_scopes(pod, &scopes) {
        return false;
    }
...
async fn create_pod(
    ...
    let pod = crate::common::build_child_pod(
        template,
        &pod_name,
        namespace,
        "",
...
async fn update_replicationcontroller_status(
    ...
    let ready_pods = active_pods
        .iter()
        .filter(|p| crate::common::is_pod_ready_value(&p.data))
        .count();
