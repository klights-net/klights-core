...
#[async_trait]
pub trait DaemonSetStore: crate::gc::GcResourceStore + Send + Sync {
...
async fn resolve_controller_revision_for_template(
    ...
    for revision in revisions {
        if !crate::common::is_owned_by(&revision.data, daemonset_uid) {
            continue;
        }
...
async fn ensure_controller_revision(
    ...
    let desired = json!({
        ...
        "metadata": {
            ...
            "labels": labels,
            "ownerReferences": [crate::common::build_owner_ref(
                "apps/v1",
                "DaemonSet",
...
async fn has_any_controller_revision(
    ...
    Ok(revisions
        .iter()
        .any(|revision| crate::common::is_owned_by(&revision.data, daemonset_uid)))
}
...
async fn next_controller_revision_number(
    ...
    let max_revision = revisions
        .iter()
        .filter(|revision| crate::common::is_owned_by(&revision.data, daemonset_uid))
...
pub async fn reconcile_daemonset(
    ...
    coordination: &crate::ControllerCoordination,
    daemonset: &Value,
    now: chrono::DateTime<chrono::Utc>,
) -> Result<()> {
    let common = crate::common::controller_common();
    ...
    let live_resource = match crate::gc::reconcile_owner_references(
        db,
        live_resource.clone(),
        pod_delete_sink,
    ...
    .await?
    {
        crate::gc::OwnerReferenceReconcile::Deleted => return Ok(()),
        crate::gc::OwnerReferenceReconcile::OwnerReferencesUpdated => {
            match db
                .get_resource("apps/v1", "DaemonSet", Some(namespace), name)
                .await?
            {
    ...
    let daemonset = crate::resource_projection::with_resource_version(
        live_resource.data,
    ...
    for pod_resource in &owned_pod_resources {
            ...
            .and_then(|n| n.as_str())
        {
            ...
            let pod_hash = pod_revision_hash(&pod_resource.data)
                .unwrap_or("")
                .to_string();
            let available = crate::common::is_pod_ready_value(&pod_resource.data);
    ...
    for node_resource in &matching_nodes {
        ...
        let mut pod = crate::common::build_child_pod(
            template,
            &pod_name,
            namespace,
            node_name,
            crate::common::OwnerInfo {
                api_version: "apps/v1",
                kind: "DaemonSet",
                name,
                uid,
...
mod tests {
    ...
    #[test]
    fn controller_revision_patch_prunes_protobuf_zero_noise() {
        let template = json!({
            "metadata": {
                "name": "",
                "namespace": "",
        ...
        assert_eq!(
            daemonset_template_patch(&template),
            json!({
                "spec": {"template": {
                    "$patch": "replace",
