...
pub struct ServiceEndpointState {
    ...
    pub endpoints: Vec<Resource>,
    pub endpoint_slices: Vec<Resource>,
}

#[async_trait]
pub trait ServicePodStore: Send + Sync {
    async fn load_service_endpoint_state(&self, namespace: &str) -> Result<ServiceEndpointState>;
}

pub async fn service_reconcile_keys_for_pod<Store: ServicePodStore + ?Sized>(
    pod: &Value,
    store: &Store,
    namespace: &str,
...
async fn service_reconcile_keys_for_pods<Store: ServicePodStore + ?Sized>(
    ...
    for service in state.services {
        ...
        let Some(selector) =
            crate::endpoints::endpoints_selector(service.data.pointer("/spec/selector"))
        else {
            continue;
        };
...
pub async fn enqueue_services_after_pod_create<Store: ServicePodStore + ?Sized>(
    pod: &Value,
    store: &Store,
...
pub async fn enqueue_services_after_pod_update<Store: ServicePodStore + ?Sized>(
    previous: &Value,
    updated: &Value,
    store: &Store,
    controller_dispatcher: &ControllerDispatcherSlot,
...
pub async fn enqueue_services_after_pod_delete<Store: ServicePodStore + ?Sized>(
    deleted: &Value,
    store: &Store,
    controller_dispatcher: &ControllerDispatcherSlot,
) -> Result<()> {
