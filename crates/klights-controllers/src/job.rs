...
#[async_trait]
pub trait JobStore: crate::gc::GcResourceStore + Send + Sync {
...
async fn delete_finished_job_for_ttl(
    ...
    let _ = crate::gc::finalize_foreground_owner_if_ready(
        db,
        &marked,
        pod_delete_sink,
...
fn preserve_job_condition_timestamps(
    ...
    crate::common::preserve_condition_update_time_if_present(condition, previous, now);
}
...
pub async fn reconcile_job(
    ...
    let reconcile_lock = coordination.reconcile_lock(
        crate::CoordinatedControllerKind::Job,
        initial_namespace,
        initial_name,
    );
    ...
    if let Some(selector) = job_pod_selector(spec, template) {
        ...
        for pod in owned_pods {
            if pod_matches_job_selector(&pod.data, &selector) {
                ...
                if crate::common::remove_owner_reference_by_uid(&mut released_pod, "Job", uid) {
