...
pub trait DeploymentStore:
    crate::gc::GcResourceStore
    + crate::deployment::DeploymentFinalizeStore
    + crate::replicaset::ReplicaSetStore
    + Send
    + Sync
{
...
struct ZeroReplicaOldReplicaSetRedrive<'a, S: ?Sized, R: ?Sized, W: ?Sized> {
    ...
    pod_reader: &'a R,
    pod_writer: &'a W,
    identity: &'a dyn crate::ControllerIdentityGenerator,
...
async fn redrive_zero_replica_old_replicasets_with_live_pods(
    ...
    for rs in owned_rs_list {
        ...
        let rs_with_metadata = crate::resource_projection::with_resource_version(
            rs.data.clone(),
            rs.resource_version,
            ctx.wall_time,
            ctx.identity,
        ...
        crate::replicaset::reconcile_replicaset(
            ...
            &rs_with_metadata,
            crate::ControllerReconcileContext::at(ctx.coordination, ctx.node_name, ctx.wall_time),
        )
        .await?;
    }
...
pub async fn reconcile_deployment(
    ...
    let live_deployment = crate::resource_projection::with_resource_version(
    ...
    if let Some(existing_rs) = matching_rs {
        ...
        if crate::gc::reconcile_owner_references(
            db,
            created_rs.clone(),
            pod_delete_sink,
            non_pod_finalization,
        ...
        .await?
            == crate::gc::OwnerReferenceReconcile::Deleted
        {
            return Ok(());
        }
