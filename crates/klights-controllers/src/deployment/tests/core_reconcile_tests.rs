...
async fn test_reconcile_deployment_status_noops_when_condition_state_unchanged() {
    ...
    let first_with_rv =
        crate::internal_test_support::inject_resource_version(first.data, first.resource_version);
    reconcile_deployment(
        &db,
        pod_repo.as_ref(),
        pod_repo.as_ref(),
    ...
    let second_with_rv =
        crate::internal_test_support::inject_resource_version(second.data, second.resource_version);
    reconcile_deployment(
        &db,
        pod_repo.as_ref(),
        pod_repo.as_ref(),
...
struct ObservedGenerationBeforeCreateWriter {
    ...
    inner: std::sync::Arc<crate::internal_test_support::TestStore>,
