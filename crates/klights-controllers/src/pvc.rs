...
#[async_trait]
pub trait PvcStore: ControllerStatusStore {
...
async fn write_pvc_status<S: PvcStore + ?Sized>(
    ...
    let updated = crate::common::write_status_for_resource(store, pvc, &status).await?;
...
pub async fn reconcile_pvc<S: PvcStore + ?Sized>(
    file_process: &klights_supervisor::FileProcessExecutor,
    local_path_provisioner_root: &std::path::Path,
    store: &S,
    pvc: &Value,
