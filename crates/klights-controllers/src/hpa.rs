...
#[async_trait]
pub trait HpaMetrics: Send + Sync {
    async fn snapshot(&self, pods: &[Resource]) -> HpaMetricsSnapshot;
}

#[async_trait]
pub trait HpaRuntime: Send + Sync {
    async fn get_hpa(
        &self,
        api_version: &str,
        namespace: &str,
...
pub async fn reconcile_hpa_with_runtime(
    ...
    let matching_ready_pods: Vec<Resource> = pods
        ...
        .filter(|pod| {
            pod.data.pointer("/metadata/deletionTimestamp").is_none()
                ...
                && crate::common::is_pod_ready_value(&pod.data)
        })
        .cloned()
        .collect();
