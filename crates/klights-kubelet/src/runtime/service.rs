...
use crate::runtime::{PodRuntimeService, PodSlotAdmissionRequest, RuntimeReconcileHint};
...
use crate::pod_container_config::{build_container_config_with_capacity, check_run_as_non_root};
...
use crate::pod_startup_error::PodStartupErrorKind;
use crate::pod_status_builders::{
    build_initial_pending_status, build_pod_initializing_app_statuses,
};
use crate::pod_termination::{find_pod_container_spec, get_termination_message_path};
use crate::runtime::active_deadline::ActiveDeadlineEnforcer;
use crate::runtime::cluster_policy::{apply_forwarded_status, owns_pod_runtime};
use crate::runtime::cri::{ContainerRuntimeControl, CriRuntime, CriRuntimeContainerEventStream};
use crate::runtime::events::PodEventSink;
use crate::runtime::filesystem::PodFilesystem;
use crate::runtime::hooks::{HookOutcome, PodHookRuntime};
use crate::runtime::hostports::HostPortRuntime;
use crate::runtime::init_container_status::{
...
use crate::runtime::pod_identity::{LivePodUidCheck, check_live_pod_uid, get_pod_for_uid};
use crate::runtime::probes::{ProbeRuntime, StartupFinalizationAction};
use crate::runtime::status_emitter::PodStatusEmitter;
...
use crate::runtime::status_projection;
...
) -> crate::runtime::cri::ContainerRuntimeState {
    if hint.event_kind(container_id) == Some(crate::cri_events::KubeletEventKind::Started)
        && matches!(
            state,
...
fn append_managed_hosts_mount(mounts: &mut Vec<k8s_cri::v1::Mount>, hosts_file_path: Option<&str>) {
    ...
    if crate::pod_hosts::container_has_etc_hosts_mount(mounts) {
        return;
    }
...
pub(crate) struct RealPodRuntimeService {
    ...
    pub(super) clock: Arc<dyn crate::runtime_clock::RuntimeClock>,
    pub(super) slot_admission: Arc<dyn PodSlotAdmission>,
    pub(super) pod_query: Arc<dyn klights_pod_api::PodQuery>,
    pub(super) pod_status_writer: Arc<dyn crate::pod_repository::status::PodStatusWriter>,
    ...
    env_source: Arc<dyn crate::pod_env::EnvSourceReader>,
...
impl RealPodRuntimeService {
    async fn enforce_active_deadline_if_exceeded(
        ...
        let Some(deadline_secs) =
            crate::runtime::active_deadline::exceeded_active_deadline_seconds_at(
                pod,
        ...
        let sandbox_id = if owns_pod_runtime(&self.config.node_name, pod) {
    ...
    /// Write Pod status through the injected focused repository ports. The
    /// worker implementation forwards through its writer; the leader writer
    /// persists locally, so runtime policy remains role-agnostic.
    ...
    pub(super) async fn build_container_config_with_env(
        ...
        let resolved_env_from = crate::pod_env::resolve_env_from_source(
            request.container,
            &request.key.namespace,
            self.env_source.as_ref(),
        )
        .await;
        let resolved_env = crate::pod_env::resolve_env_value_from_source(
            request.container,
            &request.key.namespace,
            self.env_source.as_ref(),
        )
        .await;
        let subpath_env = crate::pod_env::build_subpath_env_with_capacity(
            request.container,
            request.pod,
            &resolved_env_from,
            &resolved_env,
        ...
        let service_envs = crate::pod_service_envs::resolve_service_envs_from_source(
            &request.key.namespace,
            self.env_source.as_ref(),
        )
        .await;
        ...
        match crate::pod_volume_manager::PodVolumeManager::build_mounts(
            request.container,
    ...
    async fn restart_exited_containers_if_needed(
        ...
        let restart_policy = crate::runtime::status_helpers::pod_restart_policy(pod);
        ...
        for status in container_statuses {
            ...
            if !crate::runtime::status_helpers::should_restart_container(restart_policy, exit_code)
            {
                continue;
            }
    ...
    pub(crate) fn new(
        ...
        network: Arc<dyn PodNetworkRuntime>,
        store: Arc<dyn PodRuntimeStore>,
        clock: Arc<dyn crate::runtime_clock::RuntimeClock>,
        ...
        filesystem: Arc<dyn PodFilesystem>,
    ...
    async fn wait_for_init_container_stop(
        ...
        loop {
            ...
            if event.container_id == container_id
                && event.kind == crate::cri_events::KubeletEventKind::Stopped
            {
...
impl RealPodRuntimeService {
    pub(super) async fn stop_pod(
        ...
        let mode = if deletion_deadline.is_some() {
            ...
            crate::runtime::PodStopMode::Forced
        };
        ...
        .await?;
        match result {
            crate::runtime::PodStopResult::Completed => Ok(()),
            crate::runtime::PodStopResult::Cancelled => {
...
impl PodRuntimeService for RealPodRuntimeService {
    async fn check_slot_admission(
        ...
        crate::runtime::slot_admission::check_slot_admission(
    ...
    async fn start_pod(
        ...
        if !owns_pod_runtime(&self.config.node_name, &pod) {
            return Ok(PodStartResult::Failed(format!(
        ...
        if let Some(sandbox_id) = crate::runtime::recovery::already_realized_running_sandbox(
            self.store.as_ref(),
        ...
        for container in containers {
            ...
            let normalized_image = crate::runtime::images::normalize_image_name(image);
            let pull_policy =
                crate::runtime::images::effective_pull_policy(container, &normalized_image);
        ...
        for init_container in &init_containers {
            ...
            let normalized_image = crate::runtime::images::normalize_image_name(image);
            let pull_policy =
                crate::runtime::images::effective_pull_policy(init_container, &normalized_image);
            ...
            if exit_code != 0 {
                ...
                let restart_policy = crate::runtime::status_helpers::pod_restart_policy(&pod);
                let retry = crate::runtime::status_helpers::should_restart_container(
                    restart_policy,
                    exit_code,
                );
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    container_statuses.push(
                        crate::runtime::status_helpers::build_create_container_config_error_status(
                            container,
                            container_name,
                            &message,
                        ),
        ...
        for (container, container_id) in &started_containers {
            ...
            if let Some(post_start) = container.pointer("/lifecycle/postStart") {
                match self
                    ...
                    .await
                {
                    Ok(crate::runtime::hooks::HookOutcome::Succeeded) => {
                        tracing::debug!(container = container_name, "postStart hook succeeded");
                    }
                    Ok(crate::runtime::hooks::HookOutcome::Failed(msg)) => {
                        let _ = self
                            .events
                            .emit_pod_event(
                                &key,
    ...
    async fn stop_pod(
        ...
        if cancel.is_cancelled() {
            return Ok(crate::runtime::PodStopResult::Cancelled);
        }
        tracing::debug!(
            namespace = %key.namespace,
            pod = %key.name,
        ...
        if pod.is_none() {
            ...
            result?;
            return Ok(crate::runtime::PodStopResult::Completed);
        }
        ...
        let owned_by_this_node = owns_pod_runtime(&self.config.node_name, &pod);
        if !owned_by_this_node {
            let ownership = PodOwnershipError::from_pod_node_name(&self.config.node_name, &pod);
            let target_node = ownership.target_node.as_deref().unwrap_or("<unscheduled>");
            tracing::warn!(
                namespace = %key.namespace,
                name = %key.name,
                uid = %key.uid,
                local_node = %self.config.node_name,
            ...
            .filter(|id| seen_pre_stop_ids.insert((*id).clone()))
        {
            ...
            let remaining =
                crate::runtime::remaining_stop_grace(deletion_deadline, self.clock.now_utc(), mode);
        ...
        if let Some(sandbox_id) = sandbox_id.as_deref() {
            ...
            {
                let grace_period_seconds = crate::runtime::remaining_stop_grace_seconds(
                    deletion_deadline,
                    self.clock.now_utc(),
                    mode,
                );
    ...
    async fn finalize_startup(
        ...
        let sandbox_id = crate::runtime::startup_finalization::resolve_startup_sandbox_id(
            self.store.as_ref(),
            &key,
    ...
    async fn reconcile_cri_leftovers(&self, key: PodRuntimeKey) -> anyhow::Result<()> {
        ...
        if !owns_pod_runtime(&self.config.node_name, &resource.data) {
            return Ok(());
        }
    ...
    async fn reconcile_ephemeral(
        ...
        for ec in ephemeral_containers {
            ...
            if !image.is_empty() {
                let normalized_image = crate::runtime::images::normalize_image_name(image);
                let pull_policy =
                    crate::runtime::images::effective_pull_policy(ec, &normalized_image);
                let needs_pull = if pull_policy == "Never" {
                    false
                } else if pull_policy == "IfNotPresent" {
                    !matches!(self.cri.image_status(&normalized_image).await, Ok(true))
    ...
    async fn handle_lifecycle_command(&self, command: LifecycleCommand) -> anyhow::Result<()> {
        crate::runtime::lifecycle_commands::handle_lifecycle_command(self, command).await
    }
    ...
    async fn schedule_retry(
        ...
        crate::runtime::retry::schedule_retry(&self.supervisor, key, delay, reply_to).await
    }
    ...
    async fn schedule_start_pod_retry(
        ...
        crate::runtime::retry::schedule_start_pod_retry(
            crate::runtime::retry::RetryRuntimeContext {
                pod_status_writer: self.pod_status_writer.as_ref(),
                events: self.events.as_ref(),
                supervisor: &self.supervisor,
                node_name: &self.config.node_name,
            },
            crate::runtime::retry::StartPodRetryRequest {
                key,
                delay,
                error_message,
                attempt,
