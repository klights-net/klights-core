...
impl RealPodVolumeRuntime {
    ...
    async fn schedule_projected_sa_refresh_if_needed(
        ...
        if !crate::projected_sa_token_refresh::pod_has_projected_service_account_tokens(pod) {
        ...
        crate::projected_sa_token_refresh::schedule_projected_service_account_token_refresh(
            crate::projected_sa_token_refresh::ProjectedSaTokenRefreshRequest {
...
impl PodVolumeRuntime for RealPodVolumeRuntime {
    async fn process_volumes(
        ...
        let manager = crate::pod_volume_manager::PodVolumeManager::new(
            &self.file_process,
            self.sources.as_ref(),
            &self.paths,
            self.node_capacity,
    ...
    async fn cleanup_volumes(&self, key: &PodRuntimeKey) -> anyhow::Result<()> {
        ...
        crate::volumes::unmount_volume_mounts_under(&self.file_process, &pod_volumes_path).await?;
...
mod tests {
    ...
    async fn real_pod_volume_runtime_cleanup_removes_pod_volumes_directory() {
        ...
        let runtime_paths =
            crate::runtime_paths::KubeletRuntimePaths::new(temp.path().to_path_buf()).unwrap();

        let runtime = RealPodVolumeRuntime::new(
            crate::volume_sources::empty_volume_source_reader_for_tests(),
...
pub(crate) mod test_support {
    ...
    #[async_trait::async_trait]
    impl crate::runtime::volumes::PodVolumeRuntime for MockPodVolumeRuntime {
        async fn process_volumes(
            &self,
