...
pub(super) use crate::pod_status_builders::{
    CanonicalEphemeralContainerStatusInput as EphemeralContainerStatusInput,
    canonical_ephemeral_container_status as build_ephemeral_container_status,
};
use crate::pod_status_logic::effective_restart_policy_value;
pub(super) use crate::pod_status_logic::should_restart_container;
...
pub fn pod_restart_policy(pod: &serde_json::Value) -> &str {
    effective_restart_policy_value(
...
pub fn restart_last_state_from_runtime_status(
    ...
    let mut terminated = serde_json::json!({
        ...
        "startedAt": canonical_cri_timestamp_from_ns(
            status.map(|status| status.started_at).unwrap_or(0),
            operation_now,
        ),
        "finishedAt": canonical_cri_timestamp_from_ns(
