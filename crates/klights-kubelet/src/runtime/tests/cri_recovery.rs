use super::*;
use crate::pod_lifecycle_core::state::PodLifecycleState;
...
fn cri_runtime_trait_exposes_only_runtime_arguments() {
    ...
    assert_object_safe::<dyn crate::runtime::cri::CriRuntime>();
    assert_object_safe::<dyn crate::runtime::cri::ContainerRuntimeControl>();
}
...
fn shared_cri_runtime_clones_client_per_call_without_mutex() {
    ...
    assert_send_sync::<crate::runtime::cri::SharedCriRuntime>();
    // The adapter implements CriRuntime.
    fn _takes_cri_runtime(_: &dyn crate::runtime::cri::CriRuntime) {}
}
...
async fn real_hook_runtime_exec_hook_uses_cri_runtime_port() {
    ...
    let hooks = crate::runtime::hooks::RealPodHookRuntime::new(cri.clone(), supervisor);
...
async fn real_runtime_start_pod_rejects_uid_mismatch_before_cri() {
    ...
    let pod =
        crate::runtime::test_support::pod_json("ns", "test-pod", "correct-uid", "nginx:latest");
...
async fn start_pod_recovery_skips_already_realized_running_sandbox_with_parity() {
    ...
    let mut pod =
        crate::runtime::test_support::pod_json(&key.namespace, &key.name, &key.uid, "nginx:1.25");
...
async fn shared_cri_runtime_implements_container_runtime_control_with_parity() {
    ...
    use crate::runtime::cri::ContainerRuntimeControl;
...
async fn real_pod_runtime_store_records_and_retrieves_sandbox() {
    ...
    let store = crate::runtime::store::RealPodRuntimeStore::new(
...
async fn runtime_finalize_startup_uses_sandbox_hint_when_store_row_missing() {
    ...
    harness
        .repo
        .test_create_pod("ns", "hint-pod", "test-node", pod.clone())
        .await
        .unwrap();
...
async fn runtime_finalize_startup_uses_pod_annotation_when_store_row_missing() {
    ...
    harness
        .repo
        .test_create_pod("ns", "annotated-pod", "test-node", pod)
        .await
        .unwrap();
...
async fn finalize_startup_started_sandbox_idempotency_with_parity() {
    ...
    harness
        .repo
        .test_create_pod("ns", "idempotent-pod", "test-node", pod)
        .await
        .unwrap();
    harness
        .store
...
async fn finalize_startup_accepts_podips_startup_status_with_parity() {
    ...
    harness
        .repo
        .test_create_pod("ns", "podips-pod", "test-node", pod)
        .await
        .unwrap();
    harness
        .store
...
async fn real_runtime_reconcile_treats_cri_numeric_running_state_as_running() {
    ...
    harness.container_control.set_container_states(vec![(
        ...
        crate::runtime::cri::ContainerRuntimeState::from_cri_state_i32(
    ...
    harness
        ...
        .reconcile_runtime(
            ...
            RuntimeReconcileHint::from_container_event(
                "ctr-fast-exit",
                crate::cri_events::KubeletEventKind::Started,
            )
            .with_container_event(
                "ctr-fast-exit",
                crate::cri_events::KubeletEventKind::Stopped,
            ),
        )
        .await
        .unwrap();
...
async fn hinted_container_status_non_not_found_error_remains_fail_closed() {
    let cri = crate::runtime::test_support::MockCriRuntime::new();
