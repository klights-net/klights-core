...
async fn real_runtime_schedule_retry_emits_retry_due_after_delay() {
    ...
    let (tx, mut rx) =
        tokio::sync::mpsc::channel::<crate::pod_lifecycle_core::message::LifecycleMessage>(8);
    let reply_to = crate::pod_lifecycle_router::LifecycleReplyHandle::direct(tx);
    ...
    match message {
        crate::pod_lifecycle_core::message::LifecycleMessage::RetryDue { key } => {
...
fn pod_slot_trait_is_object_safe() {
    use crate::runtime::store::PodSlotAdmission;
...
async fn real_pod_slot_admission_admits_and_clears_slot() {
    let pod_slot_adapter = node_local_runtime_store().await;
    let admission = crate::runtime::store::RealPodSlotAdmission::new(
...
async fn runtime_stop_uses_absolute_remaining_grace_and_recomputes_per_container() {
    ...
    .await
    .unwrap();
    assert_eq!(result, crate::runtime::PodStopResult::Completed);
...
async fn runtime_stop_grace_can_exceed_cri_transport_request_timeout() {
    ...
    assert_eq!(
        crate::cri::DEFAULT_CRI_REQUEST_TIMEOUT,
...
async fn runtime_stop_elapsed_or_forced_deadline_passes_zero_and_skips_prestop() {
    for (mode, deadline_offset_ms) in [
        (crate::runtime::PodStopMode::Graceful, -1),
        (crate::runtime::PodStopMode::Forced, 30_000),
    ] {
...
async fn runtime_stop_bounds_prestop_by_deadline_and_honors_cancellation() {
    ...
    .await
    .unwrap();
    assert_eq!(result, crate::runtime::PodStopResult::Cancelled);
