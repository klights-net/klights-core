...
pub struct RealProbeRuntime {
    probe_manager: Arc<crate::probe_manager::ProbeManager>,
    started_sandboxes: Mutex<HashMap<PodRuntimeKey, StartedSandboxFinalization>>,
}

impl RealProbeRuntime {
    pub fn new(probe_manager: Arc<crate::probe_manager::ProbeManager>) -> Self {
        Self {
            probe_manager,
...
pub(crate) mod test_support {
    ...
    #[async_trait::async_trait]
    impl crate::runtime::probes::ProbeRuntime for MockProbeRuntime {
        async fn record_started_sandbox(
            &self,
            key: &crate::runtime_types::PodRuntimeKey,
            sandbox_id: &str,
        ) -> anyhow::Result<crate::runtime::probes::StartupFinalizationAction> {
            self.calls
                .lock()
                .unwrap()
            ...
            match started_sandboxes.get_mut(key) {
                ...
                {
                    Ok(crate::runtime::probes::StartupFinalizationAction::AlreadyFinalized)
                }
                Some((existing_sandbox_id, finalized)) => {
                    *existing_sandbox_id = sandbox_id.to_string();
                    *finalized = false;
                    Ok(crate::runtime::probes::StartupFinalizationAction::RunFinalizers)
                }
                None => {
