...
#[derive(Clone)]
pub struct PodLogFollowWatchSource {
    inner: Arc<dyn PodLogFollowWatchPort>,
}

impl PodLogFollowWatchSource {
    pub fn new(inner: Arc<dyn PodLogFollowWatchPort>) -> Self {
        Self { inner }
    }
}

pub trait PodLogFollowWatchPort: Send + Sync {
...
pub async fn build_log_output(
    ...
    let bytes = build_log_output_bytes_at(log_path, params, task_supervisor, operation_now).await?;
...
pub async fn build_log_output_bytes(
    ...
    task_supervisor: &klights_supervisor::TaskSupervisor,
    operation_now: time::OffsetDateTime,
) -> Result<Bytes, LogReadError> {
    build_log_output_bytes_at(log_path, params, task_supervisor, operation_now).await
}
    ...
    for attempt in 0..ATTEMPTS {
        match build_log_output_once_bytes(log_path, params, task_supervisor, operation_now).await {
            ...
            Err(e) => {
                ...
                return Err(LogReadError::Internal(
...
pub fn log_query_since_cutoff_at(
    params: &LogQuery,
...
pub fn follow_log_file_with_initial_query(
    ...
    follow_log_file_with_initial_query_at(log_path, params, task_supervisor, operation_now)
}
...
impl PodLogFollowTermination {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn new_for_test(
        pod_events: broadcast::Receiver<klights_leader_api::ResourceEvent>,
        namespace: String,
        name: String,
        uid: String,
        container_name: String,
    ...
    async fn next_event(&mut self) -> Result<klights_leader_api::ResourceEvent, PodLogEventError> {
...
enum PodLogEventSource {
    ...
    #[cfg(any(test, feature = "test-support"))]
    Broadcast(broadcast::Receiver<klights_leader_api::ResourceEvent>),
}
...
pub fn pod_log_follow_event_is_terminal(
    termination: &PodLogFollowTermination,
    ...
    let terminal_reason = match event.event_type() {
        klights_leader_api::WatchEventType::Deleted => Some("pod deleted".to_string()),
        klights_leader_api::WatchEventType::Added
        | klights_leader_api::WatchEventType::Modified => {
            pod_log_follow_terminal_reason(object, &termination.container_name)
        }
        klights_leader_api::WatchEventType::Bookmark
        | klights_leader_api::WatchEventType::Error => None,
    };
    ...
    tracing::info!(
        ...
        phase = ?object.pointer("/status/phase").and_then(|value| value.as_str()),
        terminal,
...
pub fn pod_log_follow_terminal_reason(
    pod: &serde_json::Value,
    container_name: &str,
) -> Option<String> {
    if pod
...
impl PodLogFollowTrace {
    pub fn new(mode: &'static str, target: String) -> Self {
        tracing::info!(
            target: "klights::pod_logs",
            mode,
            log_target = %target,
...
/// adapter.
pub struct LocalNodeLogRuntime {
...
impl NodeLogRuntime for LocalNodeLogRuntime {
    fn read_logs(&self, request: NodeLogRequest) -> NodeLogFuture<'_, NodeLogResult> {
        ...
        let operation_now = time::OffsetDateTime::from(self.clock.now());
