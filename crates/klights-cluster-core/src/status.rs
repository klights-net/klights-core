...
pub fn merge_status_for_apply(
    ...
    match profile.kind {
        ...
        StatusMergeProfileKind::NodeTyped => {
            merge_node_status_for_update(incoming_status, live_resource);
        }
        StatusMergeProfileKind::Generic(policy) => {
...
pub fn merge_node_status_for_update(incoming_status: &mut Value, existing: &Value) {
...
fn merge_node_status_conditions(desired: &mut Value, existing: &Value) {
    ...
    for existing_cond in existing_conditions {
        let Some(cond_type) = existing_cond.get("type").and_then(Value::as_str) else {
            continue;
        };
        match desired_conditions
            .iter()
            .position(|condition| condition.get("type").and_then(Value::as_str) == Some(cond_type))
        {
            None => desired_conditions.push(existing_cond.clone()),
            Some(index) => {
                ...
                if existing_network_pair_should_preserve && is_network_condition_type(cond_type) {
                    desired_conditions[index] = existing_cond.clone();
                    continue;
                }
                if !condition_is_strictly_newer(&desired_conditions[index], existing_cond) {
                    desired_conditions[index] = existing_cond.clone();
                }
            }
        }
...
fn is_network_condition_type(condition_type: &str) -> bool {
    matches!(condition_type, "Ready" | "NetworkUnavailable")
}

fn network_condition_pair_has_newer_transition(
    desired_conditions: &[Value],
    ...
    if desired_ready.is_some_and(|desired| {
...
fn desired_network_pair_is_coherent(ready: Option<&Value>, network: Option<&Value>) -> bool {
    matches!(
...
fn network_condition_pair_is_healthy(ready: Option<&Value>, network: Option<&Value>) -> bool {
    matches!(
        (condition_status(ready), condition_status(network)),
...
fn network_condition_pair_is_unavailable(ready: Option<&Value>, network: Option<&Value>) -> bool {
    matches!(
        (condition_status(ready), condition_status(network)),
...
fn network_condition_pair_timestamps_tie(
    desired_ready: Option<&Value>,
    desired_network: Option<&Value>,
    existing_ready: Option<&Value>,
    existing_network: Option<&Value>,
) -> bool {
...
fn condition_by_type<'a>(conditions: &'a [Value], condition_type: &str) -> Option<&'a Value> {
    conditions
        .iter()
        .find(|condition| condition.get("type").and_then(Value::as_str) == Some(condition_type))
}

fn condition_status(condition: Option<&Value>) -> Option<&str> {
    condition
        .and_then(|condition| condition.get("status"))
        .and_then(Value::as_str)
}

fn condition_transition_time(condition: Option<&Value>) -> Option<&str> {
    condition
        .and_then(|condition| condition.get("lastTransitionTime"))
        .and_then(Value::as_str)
}

fn condition_is_strictly_newer(left: &Value, right: &Value) -> bool {
    let left_time = left.get("lastTransitionTime").and_then(Value::as_str);
    let right_time = right.get("lastTransitionTime").and_then(Value::as_str);
...
mod tests {
    ...
    fn status_merge_registry_has_profiles_for_current_special_cases() {
        ...
        {
            let (api_version, kind) = ("policy/v1", "PodDisruptionBudget");
    ...
    #[test]
    fn node_status_condition_merge_is_table_driven() {
        struct Case {
            name: &'static str,
            existing: Value,
            incoming: Value,
            expected_ready: &'static str,
        }
        let cases = [
            Case {
                name: "timestamped transition replaces legacy condition",
                existing: json!({
            ...
            Case {
                name: "older incoming transition preserves live condition",
                existing: json!({
                    "status": {"conditions": [{
                        "type": "Ready",
                        "status": "Unknown",
                ...
                incoming: json!({
                    "conditions": [{
                        "type": "Ready",
                        "status": "True",
                        "lastTransitionTime": "2026-07-14T11:38:21Z"
                    }]
                }),
                expected_ready: "Unknown",
            },
        ...
        for case in cases {
            ...
            merge_node_status_for_update(&mut incoming, &case.existing);
            assert_eq!(
                incoming
                    .pointer("/conditions/0/status")
                    .and_then(Value::as_str),
                Some(case.expected_ready),
                "{}",
                case.name
            );
        }
