...
use klights_cluster_store::ReplicatedSnapshotMetadata;
...
impl Datastore {
    ...
    pub async fn replace_replicated_resource_state(
        ...
        let watch_replay_floors = watch_replay_floors.map(|floors| {
            floors
                .into_iter()
                .map(|floor| crate::sqlite::recovery::SnapshotReplayFloor {
        ...
        let metadata = metadata.map(|metadata| crate::sqlite::recovery::SnapshotMetadata {
            cluster_id: metadata.cluster_id,
            leader_epoch: metadata.leader_epoch,
            membership: match metadata.membership {
                klights_cluster_store::ReplicatedMembershipState::LegacyOmitted => {
                    crate::sqlite::recovery::SnapshotMembership::LegacyOmitted
                }
                klights_cluster_store::ReplicatedMembershipState::AuthoritativeAbsent => {
                    crate::sqlite::recovery::SnapshotMembership::AuthoritativeAbsent
                }
                klights_cluster_store::ReplicatedMembershipState::Present(membership) => {
                    crate::sqlite::recovery::SnapshotMembership::Present(membership)
                }
            },
        ...
        self.db_call_with_post_commit(
            ...
            move |conn| {
                ...
                let pending = crate::sqlite::recovery::replace_resource_state_in_conn(
                    conn,
                    entries,
                    current_rv,
            ...
            move |pending| {
                ...
                #[cfg(test)]
                if let Some(watch_bus) = watch_bus.as_deref() {
