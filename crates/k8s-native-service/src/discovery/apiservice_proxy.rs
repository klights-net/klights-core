...
pub const MAX_PROXY_RESPONSE_BODY_BYTES: usize = 32 * 1024 * 1024;
pub const MAX_APISERVICE_RESPONSE_BODY_BYTES: usize = 32 * 1024 * 1024;

fn should_forward_backend_header(name: &str) -> bool {
...
async fn read_reqwest_body_limited(
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|error| {
            AppError::BadGateway(format!("{context}: read chunk failed: {error}"))
        })?;
...
async fn find_apiservice_backend_uncached<S: DiscoveryState + ?Sized>(
    state: &S,
    group: &str,
    version: &str,
...
pub async fn find_apiservice_backend<S: DiscoveryState + ?Sized>(
    state: &S,
    group: &str,
    version: &str,
) -> Result<Option<ApiServiceBackend>, AppError> {
...
pub struct ApiServiceRequestDispatcher<S: DiscoveryState + ?Sized> {
    state: Arc<S>,
}
...
impl<S: DiscoveryState + ?Sized> ApiServiceRequestDispatcher<S> {
    ...
    pub async fn dispatch(
        ...
        if let Some(headers) = req.forward_headers {
            for (name, value) in headers {
                let key = name.as_str();
                if !should_forward_backend_header(key) {
                    continue;
                }
        ...
        let payload = read_reqwest_body_limited(
            upstream,
            MAX_APISERVICE_RESPONSE_BODY_BYTES,
            "APIService proxy",
        )
        .await?;
...
pub async fn invalidate_apiservice_proxy_cache_for_resource<S: DiscoveryState + ?Sized>(
    state: &S,
    api_version: &str,
    kind: &str,
) {
...
pub async fn resolve_service_endpoint(
    ...
    let endpoints = get_resource(query, "v1", "Endpoints", Some(namespace), service_name)
        .await?
        .ok_or_else(|| {
...
async fn cached_apiservice_proxy_client<S: DiscoveryState + ?Sized>(
    state: &S,
    backend: &ApiServiceBackend,
    service_target: &ServiceProxyTarget,
) -> Result<reqwest::Client, AppError> {
    let identity = {
        load_apiservice_proxy_identity(
            state.apiservice_proxy_cert(),
            state.apiservice_proxy_key(),
            state.task_supervisor(),
            state.apiservice_proxy_identity_cache(),
        )
        .await
    };
...
pub async fn proxy_apiservice_request<S: DiscoveryState + ?Sized>(
    state: &Arc<S>,
    group: &str,
    version: &str,
    method: Method,
