...
pub(super) fn token_review_response(mut request: Value, status: Value) -> Value {
...
pub async fn create_token_review(
    ...
    headers: HeaderMap,
    body: Bytes,
) -> Result<crate::response::K8sResponse, AppError> {
    ...
    let authentication_inputs =
        crate::current::policy_input_adapters::authentication_http_inputs(&state);
    match crate::auth_http::authenticate_token_for_review(
    ...
    .await
    {
        Ok(klights_auth::authentication::ReviewedTokenIdentity::ServiceAccount {
            claims,
            audiences,
        }) => Ok(crate::response::K8sResponse::new(
        ...
        Err(error) => Ok(crate::response::K8sResponse::new(
