...
async fn create_service(
    ...
    crate::current::quotas::check_service_type_quota(
...
async fn delete_service(
    ...
    match crate::current::resource_query_ports::get_resource(
        state.resource_mutation().resource_query.as_ref(),
        "v1",
        "Endpoints",
        Some(&namespace),
    ...
    {
        Ok(Some(endpoints)) => {
            if let Err(error) = crate::generic_command::delete_non_pod_resource(
                state.resource_mutation().resource_command.as_ref(),
                "v1",
                "Endpoints",
                Some(&namespace),
    ...
    let owner_uid = data_with_uid
        ...
        .unwrap_or("")
        .to_string();
    if let Err(e) = crate::current::gc_ports::cascade_delete(
...
async fn create_serviceaccount_token(
    ...
    let sa = crate::current::resource_query_ports::get_resource(
        state.resource_mutation().resource_query.as_ref(),
        "v1",
        "ServiceAccount",
        Some(&namespace),
    ...
    if let Some(bref) = bound_object_ref.as_ref() {
        ...
        match kind {
            ...
            "Secret" => {
                let secret = crate::current::resource_query_ports::get_resource(
                    state.resource_mutation().resource_query.as_ref(),
                    "v1",
                    "Secret",
                    Some(&namespace),
