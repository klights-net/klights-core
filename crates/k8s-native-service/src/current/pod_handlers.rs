...
pub(in crate::current) async fn list_pods(
    ...
    if query.watch == Some("true".to_string()) {
        ...
        let body = build_label_selector_watch_stream(LabelSelectorWatchStreamRequest {
            ...
            api_version: "v1",
            kind,
            watch_namespace: Some(ns.clone()),
            scope: klights_leader_api::ResourceListScope::Namespace(ns.clone()),
            requested_rv,
            send_initial_events,
            send_bookmarks,
            label_selector,
...
pub(in crate::current) async fn get_pod(
    State(state): State<Arc<ApiState>>,
    Path((namespace, name)): Path<(String, String)>,
    headers: HeaderMap,
) -> Result<K8sResponse, AppError> {
    match crate::current::pod_repository_ports::get_pod(
...
pub(in crate::current) async fn create_pod(
    State(state): State<Arc<ApiState>>,
    Path(namespace): Path<String>,
...
pub(in crate::current) async fn update_pod(
    State(state): State<Arc<ApiState>>,
    Path((namespace, name)): Path<(String, String)>,
    Query(query): Query<CreateUpdateQuery>,
    LenientJson(body): LenientJson<Value>,
...
pub(in crate::current) async fn delete_pod(
    State(state): State<Arc<ApiState>>,
    Path((namespace, name)): Path<(String, String)>,
    Query(query): Query<CreateUpdateQuery>,
    body: Bytes,
...
pub(in crate::current) async fn patch_pod(
    State(state): State<Arc<ApiState>>,
    Path((namespace, name)): Path<(String, String)>,
    Query(query): Query<CreateUpdateQuery>,
    headers: HeaderMap,
...
pub(in crate::current) async fn delete_collection_pods(
    State(state): State<Arc<ApiState>>,
    Path(namespace): Path<String>,
    Query(query): Query<DeleteCollectionQuery>,
) -> Result<Json<Value>, AppError> {
    let dry_run = crate::generic_command::DryRunMode::from_delete_collection_query(&query)?;
...
pub(in crate::current) async fn list_all_pods(
    State(state): State<Arc<ApiState>>,
    Query(query): Query<ListQuery>,
    headers: HeaderMap,
) -> Result<Response, AppError> {
...
impl UpdateStrategy for PodUpdateStrategy<'_> {
    async fn load_current(&self) -> Result<klights_cluster_core::Resource, AppError> {
        crate::current::pod_repository_ports::get_pod(
            self.state.resource_mutation().pod_repository.as_ref(),
            self.namespace,
            self.name,
        )
...
impl PatchStrategy for PodPatchStrategy<'_> {
    async fn apply_patch(
        ...
        if is_apply {
            ...
            let current = crate::current::pod_repository_ports::get_pod(
                self.state.resource_mutation().pod_repository.as_ref(),
                self.namespace,
                self.name,
            )
