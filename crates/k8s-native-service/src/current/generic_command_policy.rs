...
impl GenericCommandPolicy for ApiResourceMutationServices {
    ...
    fn prepare_create<'a>(
        ...
        Box::pin(async move {
            ...
            crate::generic_command::prepare_create_metadata(
    ...
    fn prepare_apply_create<'a>(
        ...
        Box::pin(async move {
            stamp_apply_create_identity(kind, &mut patch, identity);
            authorization
                .enforce_rbac_write_authorization(
                    resource_query,
                    identity,
...
mod tests {
    ...
    #[test]
    fn apply_create_csr_cannot_forge_spec_identity() {
        ...
        stamp_apply_create_identity("CertificateSigningRequest", &mut forged, &identity);

        assert_eq!(
            forged.pointer("/spec/username").and_then(Value::as_str),
            Some(identity.username.as_str())
        );
        assert_eq!(
            forged.pointer("/spec/uid").and_then(Value::as_str),
            Some(identity.username.as_str())
        );
        assert_eq!(
            forged.pointer("/spec/groups"),
            Some(&serde_json::json!(identity.groups))
        );
        assert!(forged.pointer("/spec/extra/forged").is_none());
    }
}
