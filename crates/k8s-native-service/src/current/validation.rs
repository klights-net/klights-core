...
#[cfg(test)]
#[path = "validation_phase17g_tests.rs"]
mod phase17g_tests;
...
pub fn validate_admissionregistration_resource(kind: &str, body: &Value) -> Result<(), AppError> {
    match kind {
        ...
        "ValidatingAdmissionPolicy" => {
            { crate::admission::validate_validating_admission_policy(body) }
...
async fn load_crd_openapi_schema(
    ...
    let crds = crate::current::resource_query_ports::list_all_resources(
        resource_query,
...
pub async fn run_admission_for_request(
    admission: &(impl crate::generic_command::ResourceAdmissionPort + ?Sized),
...
pub fn inject_resource_version(
    ...
    impl crate::ApiIdentityGenerator for FixedProjectionIdentity {
...
pub fn validate_metadata_name_for_kind(
    ...
    if kind == "IPAddress"
        || crate::current::metadata_name_uses_path_segment_validation(api_version, kind)
    {
