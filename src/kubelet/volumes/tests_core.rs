...
fn test_create_empty_dir_memory_medium() {
    ...
    let mounts = klights_supervisor::runtime_fs::read_utf8("/proc/mounts").unwrap();
    assert!(
...
async fn test_configmap_volume_items_renames_files() {
    ...
    let content =
        klights_supervisor::runtime_fs::read_utf8(format!("{}/renamed-file", path)).unwrap();
...
async fn test_configmap_volume_replaces_stale_directory() {
    ...
    let content = klights_supervisor::runtime_fs::read_utf8(&corefile_str).unwrap();
    assert!(
...
async fn test_configmap_volume_corefile_is_file_not_directory() {
    ...
    // Verify content
    let content = klights_supervisor::runtime_fs::read_utf8(&corefile_path).unwrap();
    assert!(
        content.contains("health"),
...
async fn test_configmap_volume_binary_data_and_data_combined() {
    ...
    // text file
    let text = klights_supervisor::runtime_fs::read_utf8(format!("{}/text.txt", path)).unwrap();
...
async fn test_secret_volume_items_filters_and_renames() {
    ...
    let content =
        klights_supervisor::runtime_fs::read_utf8(format!("{}/db-password", path)).unwrap();
