...
pub(in crate::api) const METRICS_API_VERSION: &str = "metrics.k8s.io/v1beta1";
const METRICS_WINDOW: &str = "30s";

pub(in crate::api) struct MetricsObjectBuilder {
    timestamp: String,
}

impl MetricsObjectBuilder {
    pub(in crate::api) fn new(timestamp: String) -> Self {
        Self { timestamp }
    }

    pub(in crate::api) fn node_metrics_object(&self, name: &str, usage: NodeMetricsUsage) -> Value {
        json!({
    ...
    pub(in crate::api) fn pod_metrics_object(&self, pod: &PodMetric) -> Value {
