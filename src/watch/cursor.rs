...
use super::filter::WatchEventFilter;
...
pub struct WatchCursor<S> {
    ...
    /// Initial watch floor from the resume/list boundary. Events at or below
    /// this RV are outside this cursor's continuity contract; unseen events
    /// above it that later fall below the advanced floor indicate an ambiguous
    /// delivery gap and must fail closed with `Expired`.
    start_floor_rv: i64,
    /// Namespace the durable replay covers, so floor-drop expiration is gated to
...
impl<S: WatchReplaySource> WatchCursor<S> {
    ...
    /// Confine floor-drop expiration to events the durable replay could contain.
            ...
            match self.rx.recv().await {
                Ok(event) => {
                    ...
                    if self.should_skip(&event) {
                        if let Some(rv) = self.skipped_live_event_requires_expired(&event) {
                            tracing::warn!(
                                ...
                                kind = event
                                    ...
                                    .and_then(|k| k.as_str())
                                    .unwrap_or(""),
                                "watch cursor expired: skipped unseen in-scope event after start floor"
                            );
                            return Err(WatchCursorError::Expired);
                        }
    ...
    fn skipped_live_event_requires_expired(&self, event: &WatchEvent) -> Option<i64> {
        let rv = event.resource_version()?;
        if rv <= self.start_floor_rv
