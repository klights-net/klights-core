...
impl RedbDatastore {
    pub(super) async fn begin_redb_snapshot(
        &self,
        ...
        supervisor
            ...
            .await?;
        let fence = self.accessor.acquire_snapshot_exclusive().await;
...
fn watermark_page(
    ...
    for entry in table.iter().map_err(persistence)? {
        ...
        rows.push(
            super::backend_impl::decode_outbox_watermark_key(key.value(), value.value())
                .map_err(corrupt)?,
        );
        next = Some(key.value().to_vec());
        if rows.len() == limit {
...
fn applied_page(
    ...
    for entry in table.iter().map_err(persistence)? {
        ...
        let row: crate::datastore::AppliedOutboxRecord =
            serde_json::from_slice(value.value()).map_err(corrupt)?;
        next = Some(key.value().to_string());
        rows.push(row.into());
        if rows.len() == limit {
...
mod tests {
    ...
    struct PausingAnchor {
        entered: Arc<tokio::sync::Notify>,
        release: Arc<tokio::sync::Notify>,
    }

    #[async_trait::async_trait]
    impl crate::datastore::backend::SnapshotCaptureAnchor for PausingAnchor {
    ...
    async fn redb_mvcc_session_pages_513_and_excludes_post_anchor_row() {
        ...
        let anchor = Arc::new(PausingAnchor {
        ...
        let capture = tokio::spawn(async move {
            capture_db
                .begin_pinned_snapshot_capture_with_anchor(
        ...
        assert!(
            ...
            "real accessor mutation must wait behind the capture fence"
        );
        release.notify_one();
        let mut session = capture.await.unwrap();
    ...
    async fn redb_mvcc_session_deadline_releases_actor_without_page_poll() {
        let db = store().await;
        let mut session = db
            .begin_pinned_snapshot_capture(request(Duration::from_millis(20)))
            .await
            .unwrap();
        ...
        db.begin_pinned_snapshot_capture(request(Duration::from_secs(1)))
            .await
            .expect("expired actor must release bounded session permit");
        drop(session);
    }
    ...
    async fn redb_snapshot_rejects_corrupt_namespace_body() {
        ...
        db.accessor
            ...
            .unwrap();
        let mut session = db
            .begin_pinned_snapshot_capture(request(Duration::from_secs(30)))
            .await
            .unwrap();
        assert!(matches!(
            session.next_page().await,
