...
async fn outbox_apply_rolls_back_mutation_when_ledger_insert_fails() {
    ...
    let err = db
        .apply_outbox_transactionally(
            "rollback-key",
            "PodStatus",
            crate::storage_wire_codec::test_outbox_command(&pod_status_payload("uid-rb")),
            "node-a",
        )
        .await
...
async fn outbox_apply_rejects_incomplete_ledger_row_without_age_based_recovery() {
    ...
    let err = db
        .apply_outbox_transactionally(
            ...
            "PodStatus",
            crate::storage_wire_codec::test_outbox_command(&pod_status_payload("uid-stale")),
            "node-a",
        )
        .await
...
async fn outbox_apply_rejects_fresh_incomplete_ledger_row_without_consuming_it() {
    ...
    let err = db
        .apply_outbox_transactionally(
            ...
            "PodStatus",
            crate::storage_wire_codec::test_outbox_command(&pod_status_payload(
