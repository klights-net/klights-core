use crate::{allocator, cli, paths, pidfile};

fn cli_flags_for_runtime(cli: cli::Cli) -> Result<Option<super::CliFlags>, String> {
    let role = match cli.node_role()? {
        Some(role) => role,
        None => return Ok(None),
    };
    let token_file = cli.token_file();
    Ok(Some(super::CliFlags {
...
pub(crate) fn main_entry() {
    ...
    if std::env::var_os("CNI_COMMAND").is_some() {
        let socket_path = klights_networking::cni_plugin::CniSocketPath::try_new(
            paths::cni_rpc_socket_path("klights")
                .to_string_lossy()
                .into_owned(),
        )
        .expect("configured CNI RPC socket path must be absolute");
        std::process::exit(klights_networking::cni_plugin::run_from_env(socket_path));
    }
    ...
    if is_runtime_command {
        ...
        let result = runtime.block_on(super::runtime::run_with_flags(flags));
    ...
    match cli.command {
        ...
        Some(cli::Command::Start) => {
            ...
            let runtime = tokio::runtime::Builder::new_multi_thread()
                ...
                .build()
                .expect("failed to build tokio runtime");
            let result = runtime.block_on(super::runtime::run_with_flags(super::CliFlags {
                rootless: cli_rootless,
                namespace: Some(cli.namespace),
                bind_address: cli.bind_address,
                anonymous_auth: cli.anonymous_auth,
                token_file: None,
                role: super::NodeRole::Leader {
                    bootstrap: super::node_role::LeaderBootstrap::Seed,
                },
            }));
            runtime.shutdown_background();
            let exit_code = match result {
        ...
        Some(cli::Command::Replica {
            ...
            let result = runtime.block_on(super::runtime::run_with_flags(super::CliFlags {
                ...
                role: super::NodeRole::Controlplane {
        ...
        Some(cli::Command::Cleanup) => {
            ...
            let runtime = tokio::runtime::Builder::new_multi_thread()
                ...
                .expect("failed to build tokio runtime");
            let result =
                runtime.block_on(super::runtime::run_cleanup_with_flags(super::CliFlags {
                    rootless: cli_rootless,
                    namespace: Some(ns),
                    bind_address: cli.bind_address,
                    anonymous_auth: cli.anonymous_auth,
...
mod tests {
    ...
    #[test]
    fn runtime_mapping_preserves_worker_startup_flags() {
        ...
        let flags = cli_flags_for_runtime(cli)
            .unwrap()
            .expect("worker command is a runtime command");
        assert!(flags.rootless);
        assert_eq!(flags.namespace.as_deref(), Some("edge"));
        assert_eq!(flags.bind_address.as_deref(), Some("127.0.0.1"));
        assert_eq!(flags.anonymous_auth, Some(false));
        assert!(matches!(flags.role, super::super::NodeRole::Worker { .. }));
    }

    #[test]
    fn stop_command_remains_outside_runtime_startup_dispatch() {
        let cli = cli::Cli::try_parse_from(["klights", "stop"]).unwrap();

        assert!(cli_flags_for_runtime(cli).unwrap().is_none());
    }
}
