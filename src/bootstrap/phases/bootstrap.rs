...
pub async fn run(args: BootstrapRunArgs<'_>) -> Result<BootstrapPhase> {
    ...
    let scheduling_mode = if has_leader_coordination {
        crate::bootstrap::composition::pod_repository::PodSchedulingMode::DeferredMultiNodeLeader
    } else {
        crate::bootstrap::composition::pod_repository::PodSchedulingMode::InlineSingleNode
    };
    ...
    let controller_dependencies = klights_controllers::ControllerRuntimeDependencies {
        ...
        network: Arc::new(
            crate::bootstrap::controller_adapters::controller_runtime_adapter::RootControllerNetworkPort::new(services.clone()),
        ),
        effects: Arc::new(
    ...
    let node_lifecycle_handle = if should_run_node_lifecycle {
        ...
        let node_lifecycle_coordination = leader_coordination
            .clone()
            .expect("leader coordination checked before node lifecycle startup");
