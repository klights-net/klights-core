...
async fn service_endpoint_batch_reconcile_creates_slice_and_endpoints_with_same_rv() {
    ...
    let endpoints = db
        .observe_endpoints("default", "batch-svc")
        .await
        .unwrap()
        .expect("Endpoints should exist");
    let slices = db
        .observe_endpoint_slices("default", Some("kubernetes.io/service-name=batch-svc"))
        .await
        .unwrap();
    let slice = slices.first().expect("EndpointSlice should exist");
...
async fn service_endpoint_batch_reconcile_is_noop_when_desired_state_matches() {
    ...
    .await
    .unwrap();
    let rv_after_first = db.endpoint_fixture_resource_version().await.unwrap();
    ...
    .await
    .unwrap();
    let rv_after_second = db.endpoint_fixture_resource_version().await.unwrap();
...
async fn test_reconcile_endpoints_skips_terminating_namespace() {
    ...
    assert!(
        db.observe_endpoints("terminating", "latency-svc")
            .await
            .unwrap()
            .is_none(),
    ...
    assert!(
        db.observe_endpoint_slice("terminating", "latency-svc-klights",)
            .await
            .unwrap()
            .is_none(),
...
async fn test_endpoint_reconciliation_dedup_updates_when_changed() {
    ...
    db.seed_endpoint_pod("test", "nginx-2", pod2).await.unwrap();
...
async fn test_reconcile_endpoints_ready_pods_in_addresses() {
    ...
    db.seed_endpoint_pod("test", "ready-pod", pod)
        .await
        .unwrap();
    ...
    let ep = db
        .observe_endpoints("test", "nginx-svc")
        .await
        .unwrap()
        .unwrap();
...
async fn test_reconcile_endpoints_and_slices_exclude_terminating_pods() {
    ...
    db.seed_endpoint_pod("test", "live-pod", live_pod)
        .await
        .unwrap();
    ...
    db.seed_endpoint_pod("test", "terminating-pod", terminating_pod)
        .await
        .unwrap();
    ...
    let slice = db
        .observe_endpoint_slice("test", "nginx-svc-klights")
        .await
        .unwrap()
        .unwrap();
...
async fn test_reconcile_endpoints_not_ready_pods_in_not_ready_addresses() {
    ...
    db.seed_endpoint_pod("test", "ready-pod", ready_pod)
        .await
        .unwrap();
    ...
    db.seed_endpoint_pod("test", "not-ready-pod", not_ready_pod)
        .await
        .unwrap();
    ...
    db.seed_endpoint_pod("test", "no-cond-pod", no_conditions_pod)
        .await
        .unwrap();
...
async fn test_reconcile_endpoints_includes_hostname_for_statefulset_pods() {
    ...
    let ep = db
        .observe_endpoints("default", "web-headless")
        .await
        .unwrap()
        .unwrap();
...
async fn test_reconcile_endpoints_splits_named_target_ports_per_resolved_port() {
    ...
    let endpoints = db
        .observe_endpoints("test", "example-named-port")
        .await
        .unwrap()
        .unwrap();
...
async fn test_mirror_endpoints_to_endpointslice_creates_matching_endpointslice() {
    ...
    let created = db
        .seed_endpoints("test", "my-service", endpoints)
        .await
        .unwrap();
    ...
    let endpointslice = db
        .observe_endpoint_slice("test", "my-service-mirror")
        .await
        .unwrap();
...
async fn test_mirror_endpoints_to_endpointslice_updates_existing_mirror() {
    ...
    let created = db
        .seed_endpoints("test", "my-service", endpoints_v1)
        .await
        .unwrap();
    let endpoints_v1 = TestAppState::endpoint_fixture_value_with_resource_version(
        created.data,
        created.resource_version,
    );
    mirror_endpoints_to_endpointslice(&db, &endpoints_v1, fixed_mirror_time())
        .await
        .unwrap();
    ...
    let updated = db
        .replace_endpoints("test", "my-service", endpoints_v2, created.resource_version)
        .await
        .unwrap();
    let endpoints_v2 = TestAppState::endpoint_fixture_value_with_resource_version(
        updated.data,
        updated.resource_version,
    );
    mirror_endpoints_to_endpointslice(&db, &endpoints_v2, fixed_mirror_time())
        .await
        .unwrap();
