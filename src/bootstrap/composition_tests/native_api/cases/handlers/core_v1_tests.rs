...
async fn service_patch_enqueues_reconcile_without_route_sync_on_request_path() {
    ...
    let (state, services) =
        crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_service_routing_observation().await;
    ...
    assert_eq!(services.sync_count(), sync_after_create);
    let drained = state
...
async fn service_create_persists_allocated_fields_in_single_service_write() {
    ...
    let mut watch_rx = state.subscribe_watch("v1", "Service");
    let app = state.router();
    ...
    assert_eq!(service_events[0].event_type.to_string(), "ADDED");
    assert_eq!(
        service_events[0]
            .object
            .pointer("/spec/clusterIP")
...
async fn service_create_releases_cluster_ip_when_nodeport_allocation_fails() {
    ...
    let state =
        crate::bootstrap::composition_tests::native_api::support::build_test_app_state().await;
    state.nodeport_exhaustion_fixture().exhaust().unwrap();
    let app = state.router();
