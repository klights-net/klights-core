...
async fn mutation_dry_run_create_does_not_persist_generated_crd_or_pod() {
    ...
    let persisted = crate::bootstrap::composition_tests::native_api::support::get_pod(
        &pod_repository,
        "default",
        "dry-run-pod",
    )
...
async fn mutation_delete_dry_run_does_not_mark_or_remove_generated_crd_or_pod() {
    ...
    let live_pod = crate::bootstrap::composition_tests::native_api::support::get_pod(
        &pod_repository,
        "default",
        "dry-run-pod",
    )
...
async fn mutation_deletecollection_dry_run_returns_status_without_deleting_any_matching_items() {
    ...
    for name in ["pod-dry-1", "pod-dry-2"] {
        let pod = crate::bootstrap::composition_tests::native_api::support::get_pod(
            &pod_repository,
            "default",
            name,
        )
...
async fn mutation_crd_events_fire_once_after_persisted_writes_and_never_on_dry_run() {
    ...
    let state =
        crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_mutation_side_effect_factory(move |hook_db| {
...
async fn mutation_create_defaults_are_persisted_for_json_and_protobuf_pod_paths() {
    ...
    let persisted_json = crate::bootstrap::composition_tests::native_api::support::get_pod(
        &pod_repository,
        "default",
        "defaults-json-pod",
    )
    ...
    let persisted_protobuf = crate::bootstrap::composition_tests::native_api::support::get_pod(
        &pod_repository,
        "default",
...
async fn create_certificate_signing_request_dispatches_csr_signer() {
    let (state, signer) = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_csr_signer_observation().await;
...
async fn foreground_delete_returns_before_synchronous_pod_cascade() {
    let (state, held_workqueue) =
        crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_held_pod_delete_workqueue().await;
