...
async fn test_task_supervisor_rejects_spoofed_remote_group_header() {
    let response = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_operational_endpoints()
        .await
        .router()
        .oneshot(
...
async fn test_api_rejects_invalid_bootstrap_bearer_token() {
    let response = crate::bootstrap::composition_tests::native_api::support::build_test_router()
        .await
        .oneshot(
            Request::get("/api")
...
async fn test_get_bootstrap_secret_returns_rotated_token_when_near_expiry() {
    ...
    crate::bootstrap::composition_tests::native_api::support::create_worker_bootstrap_token_with_ttl(
        &state.resource_store(),
        old_token,
...
async fn test_get_kube_system_nonfixed_bootstrap_secret_does_not_rotate() {
    ...
    .await
    .unwrap();
    crate::bootstrap::composition_tests::native_api::support::create_controlplane_bootstrap_token_with_ttl(
        &db,
...
async fn serviceaccount_token_request_uses_injected_api_wall_clock() {
    ...
    .unwrap();
    let state =
        crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_auth_clock(
...
async fn node_get_and_list_inject_last_heartbeat_time_only_on_raft_leader() {
    let leader = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_leader_authority().await;
