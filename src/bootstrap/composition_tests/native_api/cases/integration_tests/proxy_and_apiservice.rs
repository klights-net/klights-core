...
async fn test_tokenreview_create_validates_serviceaccount_jwt() {
    ...
    let state = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_signing_key_pem(ca_key_pem.clone()).await;
...
async fn test_tokenreview_uses_webhook_authenticator_for_opaque_tokens() {
    ...
    let state = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_authenticators(None, Some(webhook)).await;
    let app = state.router();
...
async fn test_tokenreview_omitted_audiences_validate_webhook_default_without_echoing_status() {
    ...
    let state = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_authenticators(
        None,
...
async fn test_tokenreview_reports_signing_key_dependency_failure_in_status() {
    ...
    let state = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_authentication_dependencies(
...
async fn test_tokenreview_reports_oidc_internal_failure_in_status() {
    ...
    let state =
        crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_authenticators(Some(Arc::new(FailingOidc)), None)
            .await;
    let app = state.router();
    let request = json!({
        "apiVersion": "authentication.k8s.io/v1",
...
async fn test_tokenreview_reports_bootstrap_store_failure_in_status() {
    ...
    let state = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_bootstrap_token_authenticator(Arc::new(
...
async fn test_cluster_custom_resource_watch_skips_stale_backlog_event_when_rv_zero() {
    ...
    crate::bootstrap::composition_tests::native_api::support::broadcast_watch_event(
