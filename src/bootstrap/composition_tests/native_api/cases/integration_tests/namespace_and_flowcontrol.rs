...
async fn test_delete_namespace_enters_terminating_before_final_removal() {
    ...
    crate::bootstrap::composition_tests::native_api::support::reconcile_namespace_termination(
        &db,
        "ns-delete-order",
        metrics.as_ref(),
    )
...
async fn test_base_namespace_lifecycle_adapter_deletes_non_pods_but_never_pod_rows() {
    ...
    let lifecycle =
        crate::bootstrap::composition_tests::native_api::support::namespace_lifecycle_for_test_datastore(
            db.clone(),
        );
...
async fn test_stale_namespace_termination_uid_does_not_delete_recreated_namespace_content() {
    ...
    crate::bootstrap::composition_tests::native_api::support::reconcile_namespace_termination_for_uid_with_outcome(
        &db,
        "ns-recreated",
        "old-namespace-uid",
        metrics.as_ref(),
...
async fn test_delete_collection_stale_same_name_replacement_does_not_delete_or_cascade() {
    ...
    let deleted =
        crate::bootstrap::composition_tests::native_api::support::delete_collection_listed_resource(
            &state,
            "v1",
            "ConfigMap",
            Some("stale-delete-collection-ns"),
...
async fn test_single_delete_explicit_uid_race_returns_conflict_not_notfound() {
    ...
    let err = crate::bootstrap::composition_tests::native_api::support::complete_non_foreground_delete_with_live_recheck(
        &db,
        crate::bootstrap::composition_tests::native_api::support::GeneratedDeleteCompletionRequest {
...
async fn test_apf_rejects_matching_request_when_limited_priority_level_is_full() {
    ...
    let (state, apf) = crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_priority_fairness().await;
    let db = state.resource_mutation().store.clone();
    db.create_resource(
...
async fn test_foreground_delete_mark_retries_internal_rv_conflict_without_user_rv_precondition() {
    ...
    let updated =
        crate::bootstrap::composition_tests::native_api::support::mark_foreground_deletion_with_retry(
            &db,
            "v1",
            "ReplicationController",
            Some("default"),
...
async fn test_finalizer_delete_mark_retries_internal_rv_conflict_preserving_concurrent_update() {
    ...
    let outcome = crate::bootstrap::composition_tests::native_api::support::complete_non_foreground_delete_with_live_recheck(
        &db,
        crate::bootstrap::composition_tests::native_api::support::GeneratedDeleteCompletionRequest {
            target: k8s_native_service::generic_command::ResourceDeleteTarget {
                api_version: "v1",
...
async fn test_pod_and_service_proxy_filters_sensitive_forwarded_headers() {
    ...
    let db = state.resource_mutation().store.clone();
    let bootstrap_token =
        crate::bootstrap::composition_tests::native_api::support::create_worker_bootstrap_token(
            &db,
        )
        .await
        .unwrap();
