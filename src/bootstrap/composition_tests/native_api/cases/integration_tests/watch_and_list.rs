...
async fn test_cluster_endpoints_protobuf_list_resource_version_primes_watch() {
    ...
    let list_json = k8s_native_service::test_protobuf::decode_protobuf(&list_body).unwrap();
    ...
    // Kubernetes strips the frame length and decodes the remaining bytes with
    // its raw protobuf stream serializer. Only the embedded resource uses the
    // normal k8s runtime.Unknown envelope.
    ...
    let envelope = k8s_native_service::test_protobuf::Unknown::decode(&object[4..]).unwrap();
    assert_eq!(
        envelope
            .type_meta
            .as_ref()
...
async fn test_serviceaccount_watch_modified_via_replicated_apply() {
    ...
    let sa_resource = klights_cluster_core::Resource {
        id: 0,
        api_version: "v1".into(),
    ...
    let patched = klights_cluster_core::Resource {
...
async fn test_cluster_scoped_selector_watch_from_list_rv_delivers_modified_without_pre_poll() {
    ...
    fn go_style_vapb_protobuf_body(name: &str, label_key: &str, label_value: &str) -> Vec<u8> {
        ...
        let envelope = k8s_native_service::test_protobuf::Unknown {
            type_meta: Some(k8s_native_service::test_protobuf::TypeMeta {
    ...
    assert_eq!(list_resp.status(), StatusCode::OK);
    let list_body = to_bytes(list_resp.into_body(), usize::MAX).await.unwrap();
    let list = k8s_native_service::test_protobuf::decode_protobuf(&list_body).unwrap();
    let list_rv = list
        .pointer("/metadata/resourceVersion")
        .and_then(|value| value.as_str())
    ...
    let mut updated = k8s_native_service::test_protobuf::decode_protobuf(&get_body).unwrap();
    ...
    let update_resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    k8s_native_service::test_protobuf::encode_protobuf(&updated).unwrap(),
                ))
                .unwrap(),
        )
        .await
...
async fn test_serviceaccount_watch_modified_survives_broadcast_lag() {
    ...
    for i in 0..1200 {
        rv += 1;
        let other = klights_cluster_core::Resource {
            id: 0,
            api_version: "v1".into(),
            kind: "ServiceAccount".into(),
...
async fn test_namespaced_crd_watch_field_selector_accepts_declared_selectable_fields() {
    ...
    let selectable_fields = state
        .crd_selectable_fields("stable.example.com", "v2", plural)
        .await
        .expect("v2 CRD must be registered");
    assert!(
        selectable_fields.contains(&"host".to_string()),
        "v2 selectable fields must include host, got {:?}",
        selectable_fields
    );
    assert!(
        selectable_fields.contains(&"port".to_string()),
    ...
    let object_watch_resp = app
        .clone()
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(format!(
                    "/apis/stable.example.com/v1/namespaces/default/{plural}?watch=true&fieldSelector=hostPort%3Dhost1%3A80"
                ))
                .body(Body::empty())
                .unwrap(),
        )
        .await
        .unwrap();
    assert_eq!(object_watch_resp.status(), StatusCode::OK);

    let create_object = app
        .clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/apis/stable.example.com/v1/namespaces/default/{plural}"
                ))
                .header("content-type", "application/json")
                .body(Body::from(
                    serde_json::to_vec(&json!({
        ...
        .await
        .unwrap();
    assert_eq!(create_object.status(), StatusCode::CREATED);

    let mut object_stream = object_watch_resp.into_body().into_data_stream();
    let chunk = tokio::time::timeout(Duration::from_secs(3), object_stream.next())
        .await
        .expect("selectable-field watch must deliver its matching custom resource")
        .expect("selectable-field watch must remain open")
        .expect("selectable-field watch chunk must decode");
    let event: serde_json::Value = serde_json::from_slice(&chunk).unwrap();
    assert_eq!(event["type"], "ADDED");
    assert_eq!(event["object"]["apiVersion"], "stable.example.com/v1");
    assert_eq!(event["object"]["kind"], "SelectableFieldCRD");
    assert_eq!(event["object"]["metadata"]["name"], "host-one");
...
async fn test_cluster_custom_resource_watch_accepts_hyphenated_kind() {
    let app = build_test_app_state().await.router();
...
///      pod state.
///
/// Focused native unit coverage exercises `watch_event_to_table` in isolation.
...
async fn test_guestbook_selector_pod_list_returns_fresh_running_status_json_and_protobuf() {
    ...
    async fn list_pods(
        ...
        if accept.is_some_and(|value| value.contains("protobuf")) {
            ...
            k8s_native_service::test_protobuf::decode_protobuf(&body).unwrap()
        } else {
...
async fn test_selector_watch_catch_up_same_rv_filtered_event_does_not_hide_matching_pod() {
    ...
    db.apply_resource_batch(vec![
        klights_cluster_core::ResourceBatchOperation::Put {
            api_version: "v1".to_string(),
            kind: "Pod".to_string(),
            namespace: Some(namespace.to_string()),
            name: "backend".to_string(),
            data: filtered,
            mode: klights_cluster_core::ResourceBatchPutMode::Create,
            preconditions: klights_cluster_core::ResourcePreconditions::default(),
        },
        klights_cluster_core::ResourceBatchOperation::Put {
            api_version: "v1".to_string(),
            kind: "Pod".to_string(),
...
async fn test_guestbook_selector_watch_observes_raft_pod_status_outbox_update() {
    ...
    let command = klights_cluster_core::command::StorageCommand::UpdateStatus {
        ...
        preconditions: klights_cluster_core::ResourcePreconditions::from_resource(&created),
    ...
    assert!(
        apply.terminal_rejection().is_none(),
        "V1 raft status commit must apply without an error: {:?}",
        apply.terminal_rejection()
    );
...
#[tokio::test]
async fn test_guestbook_selector_watchlist_delivers_ready_after_initial_events_end() {
    use axum::body::Body;
    use axum::http::{Request, StatusCode};
    use futures::StreamExt;
    use serde_json::json;
    ...
    let (app, db) = build_test_router_with_db().await;
    let namespace = "gb-watchlist-ready";
    let selector = "app%3Dguestbook%2Ctier%3Dfrontend";
    let pods = [
        ("frontend-a", "mn-replica", "10.50.3.2"),
        ("frontend-b", "mn-worker", "10.50.4.2"),
        ("frontend-c", "mn-worker2", "10.50.5.2"),
    ];
    ...
    'initial_events: loop {
        assert!(
            ...
            "watch-list must emit initial-events-end bookmark"
        );
        let chunk = tokio::time::timeout(Duration::from_millis(500), stream.next())
            .await
            .expect("watch-list chunk before initial-events-end")
            .expect("watch-list stream ended before initial-events-end")
            .expect("watch-list chunk must be ok");
        for line in String::from_utf8(chunk.to_vec())
            .unwrap_or_default()
            .lines()
            .filter(|line| !line.trim().is_empty())
        {
            ...
            if event["type"] == "BOOKMARK"
                && event.pointer("/object/metadata/annotations/k8s.io~1initial-events-end")
                    == Some(&json!("true"))
            {
                assert_eq!(
                    initial,
                    pods.iter()
                        ...
                        .collect::<BTreeSet<_>>(),
                    "watch-list initial stream must include every frontend Pod"
                );
                break 'initial_events;
            }
        }
    }
    ...
    while tokio::time::Instant::now() < deadline {
        let chunk = match tokio::time::timeout(Duration::from_millis(500), stream.next()).await {
            Ok(Some(Ok(bytes))) => bytes,
            Ok(Some(Err(err))) => panic!("watch-list stream failed: {err}"),
            Ok(None) => break,
            Err(_) => continue,
        };
        for line in String::from_utf8(chunk.to_vec())
            ...
            .filter(|line| !line.trim().is_empty())
        {
            ...
            if event.pointer("/object/status/phase") != Some(&json!("Running")) {
                continue;
            }
        ...
        if ready.len() == pods.len() {
            return;
        }
    }

    panic!(
        "watch-list stream must deliver Running MODIFIED events for every frontend Pod; got {ready:?}"
    );
}
...
async fn test_selector_watch_from_list_rv_delivers_baseline_delete_below_floor() {
    ...
    let cm = klights_cluster_core::Resource {
        id: 0,
        api_version: "v1".into(),
        kind: "ConfigMap".into(),
...
#[tokio::test]
async fn test_protobuf_selector_watch_delivers_actor_finalized_stateful_pods_in_ordinal_order() {
    use axum::body::{Body, to_bytes};
    use axum::http::{Request, StatusCode};
    use futures::StreamExt;
    use k8s_native_service::test_protobuf::Message;
    ...
    let app = state.router();
    let db = state.resource_mutation().store.clone();
    let namespace = "stateful-watch-order";
    ...
    for ordinal in 0..3 {
        let name = format!("ss-{ordinal}");
        let uid = format!("stateful-watch-uid-{ordinal}");
        db.create_resource(
            "v1",
            "Pod",
            Some(namespace),
            ...
            json!({
                ...
                "spec": {
                    "nodeName": "worker-a",
                    "containers": [{"name": "webserver", "image": "example.invalid/httpd"}]
                },
                "status": {"phase": "Running"}
            }),
        )
    ...
    let list = app
        ...
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/namespaces/{namespace}/pods?labelSelector=baz%3Dblah%2Cfoo%3Dbar"
                ))
                .header("accept", "application/vnd.kubernetes.protobuf")
                .body(Body::empty())
                .unwrap(),
    ...
    let list = k8s_native_service::test_protobuf::decode_protobuf(
    ...
    let list_rv = list
        ...
        .expect("PodList resourceVersion");
    ...
    let watch = app
        ...
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/namespaces/{namespace}/pods?allowWatchBookmarks=true&labelSelector=baz%3Dblah%2Cfoo%3Dbar&resourceVersion={list_rv}&watch=true"
                ))
                .header("accept", "application/vnd.kubernetes.protobuf")
                .body(Body::empty())
                .unwrap(),
    ...
    for (name, uid) in identities.iter().rev() {
        let delete = app
            .clone()
            .oneshot(
                Request::builder()
                    .method("DELETE")
                    .uri(format!("/api/v1/namespaces/{namespace}/pods/{name}"))
                    .header("content-type", "application/json")
                    .body(Body::from(r#"{"gracePeriodSeconds":0}"#))
                    .unwrap(),
            )
            .await
            .unwrap();
        assert!(
            ...
            "Pod DELETE must mark {name} terminating: {}",
            delete.status()
        );
        assert!(
            state
                ...
                .await
                .unwrap(),
            "actor finalization must accept {name}"
        );
    }
    ...
    let observed = tokio::time::timeout(Duration::from_secs(3), async {
        ...
        while deleted.len() < 3 {
            let chunk = stream
                .next()
                .await
                .expect("Pod watch must remain live")
                .expect("Pod watch chunk");
            pending.extend_from_slice(&chunk);
            while pending.len() >= 4 {
                let frame_len = u32::from_be_bytes(pending[..4].try_into().unwrap()) as usize;
                if pending.len() < frame_len + 4 {
                    break;
                }
                let event = k8s_pb::apimachinery::pkg::apis::meta::v1::WatchEvent::decode(
                    &pending[4..frame_len + 4],
                )
                .unwrap();
                pending.drain(..frame_len + 4);
                if event.r#type.as_deref() != Some("DELETED") {
                    continue;
                }
                let object = event.object.and_then(|object| object.raw).unwrap();
                let envelope =
                    k8s_native_service::test_protobuf::Unknown::decode(&object[4..]).unwrap();
                let pod = k8s_pb::api::core::v1::Pod::decode(envelope.raw.as_slice()).unwrap();
                deleted.push(pod.metadata.and_then(|metadata| metadata.name).unwrap());
            }
        }
        deleted
    })
    .await
    .expect("selector watch must deliver all actor-owned Pod deletions");

    assert_eq!(observed, ["ss-2", "ss-1", "ss-0"]);
}
...
async fn test_crd_live_replay_failure_emits_terminal_error() {
    ...
    let (state, failing_history) =
        crate::bootstrap::composition_tests::native_api::support::build_test_app_state_with_toggle_failing_watch_history().await;
...
async fn test_rv_selector_cr_watch_delivers_baseline_delete_below_floor() {
    ...
    let cr = klights_cluster_core::Resource {
        id: 0,
        api_version: "selwatch.example.com/v1".into(),
        kind: "Selw".into(),
        namespace: Some(namespace.into()),
