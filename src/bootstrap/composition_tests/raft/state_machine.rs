mod tests {
    ...
    async fn state_machine_with_activation(
        ...
        IntegrationRaftComposition::new(backend)
            .state_machine(applied_state.clone(), applied_state, supervisor)
            .await
    }
    ...
    async fn disk_snapshot_fence_covers_applied_state_anchor_and_capture_pin() {
        ...
        let (mut state_machine, _) = IntegrationRaftComposition::new(backend)
            .state_machine(applied_state, paused_applied_state.clone(), supervisor)
            .await;
        let mut builder = state_machine.get_snapshot_builder().await;
        let snapshot_task = tokio::spawn(async move { builder.build_snapshot().await });
        tokio::time::timeout(
    ...
    fn resource_fingerprint_row(
        ...
        (
            ...
            IntegrationRaftComposition::inject_resource_version(
                resource.data,
                resource.resource_version,
            ),
        )
