...
mod cases {
    ...
    use klights_node_api::{
        ExecStreamChannel, ExecStreamOptions, NodeExecFrame, NodeExecRequest,
        NodeExecRuntimeFuture, NodeExecSession, NodeExecSyncRequest, NodeExecSyncResult,
        NodeExecTarget, NodeLogOptions, NodeLogRequest, NodeLogTarget, NodeMetricsFuture,
        NodeMetricsRequest, NodeMetricsRuntime, NodeMetricsTarget,
    };
    use klights_node_api::{
    ...
    fn unary_deadline_policy(
        ...
        klights_leader_rpc::transport_policy::GrpcTransportPolicy {
    ...
    impl TlsGrpcLeaderFixture {
        async fn start() -> Self {
            ...
            .unwrap();
            let token =
                crate::bootstrap::composition_tests::leader_rpc::support::ensure_worker_bootstrap_token(db.as_ref())
                    .await
                    .unwrap();
    ...
    async fn grpc_watch_gate_server_with_policy(
        ...
        let app = crate::bootstrap::composition_tests::leader_rpc::support::mount_service_with_passive_reads_and_leader_gate(
            axum::Router::new(),
            service,
            db.clone(),
            passive_reads,
        ...
        let (endpoint, handle) =
            crate::bootstrap::composition_tests::leader_rpc::support::serve_tls_test_app(app).await;
    ...
    async fn grpc_hanging_watch_server() -> (String, tokio::task::JoinHandle<()>) {
        ...
        let app = crate::bootstrap::composition_tests::leader_rpc::support::mount_service_full(
            axum::Router::new(),
            service,
            db,
            None,
    ...
    async fn watch_open_tries_next_endpoint_after_not_raft_leader() {
        ...
        let token = crate::bootstrap::composition_tests::leader_rpc::support::ensure_worker_bootstrap_token(
            leader_db.as_ref(),
        )
        .await
        .expect("read token");
    ...
    async fn every_unary_rpc_is_bounded_by_per_call_deadline() {
        ...
        let controlplane_registration =
            klights_kubelet::node::NodeRegistrationSnapshot::capture_local(
                &crate::bootstrap::composition_tests::leader_rpc::support::test_file_process_executor(),
                "cp2",
                &profile,
    ...
    async fn local_pod_log_follow_closes_on_matching_pod_deleted_event() {
        ...
        crate::bootstrap::composition_tests::leader_rpc::support::seed_namespace(
            pod_event_db.as_ref(),
            "sonobuoy",
        )
        .await;
