...
async fn remote_exec_fixture() -> (
    crate::bootstrap::native_api_composition::support::IntegrationFollowerSession,
    crate::bootstrap::native_api_composition::support::RemoteExecSyncWebSocketFixture,
) {
    let state = support::build_test_app_state_with_operational_endpoints().await;
    let follower = state
