...
async fn validate_sa_token_bindings(
    ...
    crate::bootstrap::native_api_composition::support::validate_sa_token_bindings(state, claims)
        .await
}
...
async fn authorization_denial_writes_structured_audit_event() {
    ...
    let authorizer: Arc<dyn klights_auth::authorizer::Authorizer> = Arc::new(
        crate::bootstrap::native_api_composition::support::RecordingAuthorizer::deny(
            "policy denied secret read",
        ),
    );
    let state = crate::bootstrap::native_api_composition::support::build_test_app_state_with_authorizer_and_audit_sink(
        authorizer,
        audit_sink.clone(),
    )
    .await;
...
async fn pod_exec_authorization_writes_high_value_audit_event() {
    ...
    let authorizer: Arc<dyn klights_auth::authorizer::Authorizer> =
        Arc::new(crate::bootstrap::native_api_composition::support::RecordingAuthorizer::allow());
    let state = crate::bootstrap::native_api_composition::support::build_test_app_state_with_authorizer_and_audit_sink(
        authorizer,
        audit_sink.clone(),
    )
        ...
        for (accept, expected_content_type) in encodings {
            let state = match failure.failure {
                Failure::ForbiddenImpersonation => {
                    let authorizer: Arc<dyn klights_auth::authorizer::Authorizer> = Arc::new(
                        crate::bootstrap::native_api_composition::support::RecordingAuthorizer::deny("impersonation denied"),
                    );
                    crate::bootstrap::native_api_composition::support::build_test_app_state_with_authorizer(authorizer).await
                }
                Failure::AuthorizationDenied => {
                    let authorizer: Arc<dyn klights_auth::authorizer::Authorizer> = Arc::new(
                        crate::bootstrap::native_api_composition::support::RecordingAuthorizer::deny("authorization denied"),
                    );
                    crate::bootstrap::native_api_composition::support::build_test_app_state_with_authorizer(authorizer).await
                }
                Failure::InvalidAuthorization | Failure::InvalidImpersonation => {
                    crate::bootstrap::native_api_composition::support::build_test_app_state().await
                }
            };
            let app = state.router();
