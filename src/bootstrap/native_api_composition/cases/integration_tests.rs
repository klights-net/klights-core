...
fn test_protobuf_decode_preserves_configmap_data_field() {
    ...
    let unknown = k8s_native_service::test_protobuf::Unknown {
    ...
    let json_value = k8s_native_service::test_protobuf::decode_protobuf(&envelope_buf).unwrap();
...
#[tokio::test]
async fn test_api_build_state_exposes_no_service_routing_white_box() {
    let (_state, services) = build_test_app_state_with_service_routing_observation().await;
    assert_eq!(services.sync_count(), 0);
    assert_eq!(services.sync_now_count(), 0);
}
...
async fn test_delete_controller_managed_endpointslice_queues_service_reconcile() {
    ...
    .await
    .expect("queued Service reconcile should restore the managed EndpointSlice");
    cancel.cancel();
...
async fn test_replicationcontroller_scale_put_triggers_reconcile() {
    ...
    let stored_scale = db
        ...
        .unwrap()
        .unwrap();
    assert_eq!(stored_scale.data["spec"]["replicas"], json!(3));
    assert_eq!(
        state
            .controller_runtime_fixture()
            .queued_reconcile_keys()
...
async fn test_replicationcontroller_main_put_triggers_reconcile() {
    ...
    let stored_update = db
        .get_resource("v1", "ReplicationController", Some("default"), "rc-put-e2e")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(stored_update.data["spec"]["replicas"], json!(3));
    assert_eq!(
        state
            .controller_runtime_fixture()
            .queued_reconcile_keys()
...
async fn test_replicationcontroller_protobuf_create_defaults_selector_from_template_labels() {
    ...
    let wire = k8s_native_service::test_protobuf::encode_protobuf(&rc_body)
        .expect("RC protobuf encode must work");
...
async fn test_pod_lifecycle_debug_endpoint_returns_snapshot_shape() {
    ...
    let key = klights_kubelet::pod_lifecycle_actor::message::PodLifecycleKey::new(
        "default",
        "lifecycle-debug-pod",
        "uid-123",
    );
    let router = std::sync::Arc::new(
        klights_kubelet::pod_lifecycle_router::PodLifecycleRouter::new_test_default(
            supervisor,
    ...
    let _ = router
        .route(
            klights_kubelet::pod_lifecycle_actor::message::LifecycleMessage::WatchAdded {
                key: key.clone(),
        ...
        .await;
    let app_state =
        crate::bootstrap::native_api_composition::support::build_test_app_state_with_pod_lifecycle_diagnostics(router).await;
...
mod authentication_boundaries;
mod operational_composition;
