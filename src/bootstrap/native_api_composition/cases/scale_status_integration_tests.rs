...
async fn patch_statefulset_scale_does_not_conflict_with_concurrent_status_updates() {
    ...
    let pause = state.install_resource_mutation_pause(
        crate::bootstrap::native_api_composition::support::IntegrationResourceMutationPauseOperation::BuildPatchCommand,
        "apps/v1",
        "StatefulSet",
        Some("default"),
        "scale-race",
...
async fn update_replicaset_scale_with_stale_resource_version_returns_conflict() {
    ...
    let (app, db) =
        crate::bootstrap::native_api_composition::support::build_test_router_with_db().await;
    db.create_resource(
        "apps/v1",
        "ReplicaSet",
        Some("default"),
