...
async fn test_apiservice_status_reconciles_after_backend_service_and_endpoints_create() {
        ...
        loop {
            let status = app
                ...
                .oneshot(
                    Request::builder()
                        .method("GET")
                        .uri("/apis/apiregistration.k8s.io/v1/apiservices/v1alpha1.ready.example.com/status")
                        .body(Body::empty())
                        .unwrap(),
                )
                .await
            ...
            .unwrap();
            if apiservice_available_condition(&body)
    ...
    .await
    .expect("APIService reconcile should observe its backend");
    cancel.cancel();
...
async fn test_tokenreview_create_validates_serviceaccount_jwt() {
    ...
    let state = crate::bootstrap::native_api_composition::support::build_test_app_state_with_signing_key_pem(ca_key_pem.clone()).await;
    ...
    for content_type in ["application/json", "application/vnd.kubernetes.protobuf"] {
        let request_body = if content_type == "application/json" {
            ...
            k8s_native_service::test_protobuf::encode_protobuf(&req_body).unwrap()
        };
        let resp = app
            .clone()
            .oneshot(
    ...
    let other_audience_token = klights_auth::generate_sa_token_with_bound_pod_at(
        klights_auth::ServiceAccountTokenRequest {
            ca_key_pem: &ca_key_pem,
            service_account: "default",
            namespace: "kube-system",
...
async fn test_tokenreview_unauthenticated_response_supports_protobuf() {
    ...
    for content_type in ["application/json", "application/vnd.kubernetes.protobuf"] {
        let request_body = if content_type == "application/json" {
            ...
            k8s_native_service::test_protobuf::encode_protobuf(&request).unwrap()
        };
        let response = app
            .clone()
            .oneshot(
...
async fn test_tokenreview_uses_webhook_authenticator_for_opaque_tokens() {
    ...
    let webhook = Arc::new(klights_auth::webhook_auth::WebhookAuth::new(
    ...
    let state = crate::bootstrap::native_api_composition::support::build_test_app_state_with_authenticators(None, Some(webhook)).await;
    let app = state.router();
...
async fn test_tokenreview_omitted_audiences_validate_webhook_default_without_echoing_status() {
    ...
    let state = crate::bootstrap::native_api_composition::support::build_test_app_state_with_authenticators(
        None,
        Some(Arc::new(RecordingWebhook { seen: seen.clone() })),
    )
    .await;
    let app = state.router();
...
async fn test_tokenreview_uses_oidc_authenticator_for_jwt_tokens() {
    ...
    let state = crate::bootstrap::native_api_composition::support::build_test_app_state_with_authenticators(
        Some(Arc::new(StaticOidcValidator)),
        None,
    )
    .await;
    let app = state.router();
    ...
    for content_type in ["application/json", "application/vnd.kubernetes.protobuf"] {
        let request_body = if content_type == "application/json" {
            ...
            k8s_native_service::test_protobuf::encode_protobuf(&omitted_request).unwrap()
        };
        let response = app
            .clone()
            .oneshot(
...
async fn test_tokenreview_reports_signing_key_dependency_failure_in_status() {
    ...
    let state = crate::bootstrap::native_api_composition::support::build_test_app_state_with_authentication_dependencies(
        Arc::new(MissingSigningKey),
        Some(Arc::new(RejectingOidc)),
        Some(Arc::new(RejectingWebhook)),
    )
    .await;
    let app = state.router();
    let request = json!({
...
async fn test_tokenreview_reports_oidc_internal_failure_in_status() {
    ...
    let state =
        crate::bootstrap::native_api_composition::support::build_test_app_state_with_authenticators(Some(Arc::new(FailingOidc)), None)
            .await;
    let app = state.router();
    let request = json!({
        "apiVersion": "authentication.k8s.io/v1",
...
async fn test_tokenreview_reports_bootstrap_store_failure_in_status() {
    ...
    let state = crate::bootstrap::native_api_composition::support::build_test_app_state_with_bootstrap_token_authenticator(Arc::new(
    ...
    let create_crd = app
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    k8s_native_service::test_protobuf::encode_protobuf(&crd).unwrap(),
                ))
                .unwrap(),
        )
        .await
...
async fn test_crd_get_non_storage_version_uses_conversion_webhook() {
    ...
    .await
    .unwrap();
    state.register_crd_value(&crd).await.unwrap();
...
async fn test_crd_conversion_service_webhook_uses_endpoint_target_port() {
    ...
    tokio::spawn(async move {
        let hit = service_listener.accept().await.is_ok();
        let _ = service_hit_tx.send(hit);
    });
    let (endpoint_hit_tx, endpoint_hit_rx) = oneshot::channel();
    tokio::spawn(async move {
        let hit = endpoint_listener.accept().await.is_ok();
    ...
    db.create_resource(
        ...
        json!({
            ...
            "spec": {"ports": [{"port": service_port, "targetPort": endpoint_port}]}
        }),
    )
    .await
    .unwrap();
    ...
    let response = state
        .router()
        .oneshot(
            Request::get("/apis/example.com/v2/namespaces/default/widgets/w1")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert!(matches!(
        timeout(Duration::from_secs(2), endpoint_hit_rx).await,
        Ok(Ok(true))
    ));
    assert!(!matches!(
        timeout(Duration::from_millis(100), service_hit_rx).await,
        Ok(Ok(true))
    ));
}
...
async fn test_cluster_custom_resource_watch_skips_stale_backlog_event_when_rv_zero() {
    ...
    crate::bootstrap::native_api_composition::support::broadcast_watch_event(
        state.commit_watch_fixture().as_ref(),
        old_object.clone(),
    );
...
async fn custom_resource_operations_denied_with_deny_authorizer() {
    ...
    state
        .register_crd_info(klights_controllers::crd::CrdResourceInfo {
...
async fn custom_resource_allowed_identity_gets_normal_response() {
    ...
    let authorizer: std::sync::Arc<dyn klights_auth::authorizer::Authorizer> =
        std::sync::Arc::new(crate::bootstrap::native_api_composition::support::AllowAllAuthorizer);
    let state = build_test_app_state_with_authorizer(authorizer).await;
