...
async fn test_cronjob_status_put_protobuf_updates_last_schedule_time() {
    ...
    let protobuf_body = k8s_native_service::test_protobuf::encode_protobuf(&update_body)
...
async fn test_job_status_put_protobuf_updates_existing_custom_condition_timestamp() {
    ...
    let protobuf_body = k8s_native_service::test_protobuf::encode_protobuf(&update_body)
        .expect("Job status update encodes");
    let req = Request::builder()
        .method("PUT")
        .uri("/apis/batch/v1/namespaces/job-status-protobuf/jobs/job1/status")
        .header("content-type", "application/vnd.kubernetes.protobuf")
...
async fn test_delete_namespace_enters_terminating_before_final_removal() {
    ...
    crate::bootstrap::native_api_composition::support::reconcile_namespace_termination(
        &db,
        "ns-delete-order",
        metrics.as_ref(),
    )
...
#[tokio::test]
async fn test_base_namespace_lifecycle_adapter_deletes_non_pods_but_never_pod_rows() {
    use serde_json::json;

    let state = build_test_app_state().await;
    let db = state.resource_mutation().store.clone();
    let namespace = "base-owned-namespace-adapter";
    ...
    db.create_namespace(
        ...
        json!({
            "apiVersion": "v1",
            "kind": "Namespace",
            "metadata": {"name": namespace, "uid": "base-adapter-ns-uid"},
            "spec": {"finalizers": ["kubernetes"]},
            "status": {"phase": "Active"}
        }),
    )
    ...
    db.create_resource(
        ...
        json!({
            "apiVersion": "v1",
            "kind": "ConfigMap",
            "metadata": {"name": "ordinary-content", "namespace": namespace}
        }),
    )
    .await
    .unwrap();
    db.create_resource(
        ...
        json!({
            ...
            "kind": "Pod",
            "metadata": {
                "name": "actor-owned-pod",
                "namespace": namespace,
                "uid": "actor-owned-pod-uid"
            },
            "spec": {"nodeName": "worker-a", "containers": []}
        }),
    )
    ...
    let lifecycle =
        crate::bootstrap::native_api_composition::support::namespace_lifecycle_for_test_datastore(
            db.clone(),
        );
    let non_pod = lifecycle
        .list_namespace_non_pod_resources(namespace.to_string())
        .await
        .unwrap()
        .into_iter()
        .find(|resource| resource.kind == "ConfigMap")
        .expect("base adapter must list namespace non-Pod content");
    lifecycle
        .delete_namespace_non_pod_resource(non_pod, namespace.to_string())
        .await
        .unwrap();

    assert!(
        db.get_resource("v1", "ConfigMap", Some(namespace), "ordinary-content")
            .await
            .unwrap()
            .is_none(),
        "the base-owned adapter must expose non-Pod namespace cleanup"
    );
    let pod = db
        .get_resource("v1", "Pod", Some(namespace), "actor-owned-pod")
        .await
        .unwrap()
        .expect("namespace lifecycle must not hard-delete a Pod row");
    let error = lifecycle
        .delete_namespace_non_pod_resource(pod.clone(), namespace.to_string())
        .await
        .expect_err("the non-Pod capability must reject a Pod hard-delete attempt");
    assert!(
        matches!(
            error,
            klights_reconcile_api::NamespaceLifecycleError::Internal { .. }
        ),
        "the fixed non-Pod capability must fail closed for a Pod: {error:?}"
    );
    assert!(
        db.get_resource("v1", "Pod", Some(namespace), "actor-owned-pod")
            .await
            .unwrap()
            .is_some(),
        "a rejected generic Pod delete must leave the actor-owned row intact"
    );
}
...
async fn test_stale_namespace_termination_uid_does_not_delete_recreated_namespace_content() {
    ...
    crate::bootstrap::native_api_composition::support::reconcile_namespace_termination_for_uid_with_outcome(
        &db,
        "ns-recreated",
        "old-namespace-uid",
        metrics.as_ref(),
...
async fn test_delete_configmap_with_finalizer_marks_terminating_without_hard_delete() {
    ...
    use axum::http::{Request, StatusCode};
    use klights_cluster_store::WatchTarget;
    use tower::ServiceExt;
...
async fn test_delete_collection_stale_same_name_replacement_does_not_delete_or_cascade() {
    ...
    db.delete_resource_with_preconditions(
        ...
        Some("stale-delete-collection-ns"),
        "same-name",
        klights_cluster_core::ResourcePreconditions::uid("old-owner-uid"),
    )
    .await
    .unwrap();
    db.create_resource(
    ...
    let deleted =
        crate::bootstrap::native_api_composition::support::delete_collection_listed_resource(
            &state,
            "v1",
            "ConfigMap",
            Some("stale-delete-collection-ns"),
...
async fn test_single_delete_explicit_uid_race_returns_conflict_not_notfound() {
    ...
    db.delete_resource_with_preconditions(
        ...
        Some("default"),
        "cm-explicit-uid-race",
        klights_cluster_core::ResourcePreconditions::uid("old-uid"),
    )
    .await
    .unwrap();
    db.create_resource(
    ...
    let err = crate::bootstrap::native_api_composition::support::complete_non_foreground_delete_with_live_recheck(
        &db,
        crate::bootstrap::native_api_composition::support::GeneratedDeleteCompletionRequest {
            ...
            delete_preconditions: klights_cluster_core::ResourcePreconditions::uid("old-uid"),
    ...
    assert!(
        matches!(err, k8s_native_service::AppError::Conflict(_)),
...
async fn test_apf_rejects_matching_request_when_limited_priority_level_is_full() {
    ...
    let (state, apf) = crate::bootstrap::native_api_composition::support::build_test_app_state_with_priority_fairness().await;
    let db = state.resource_mutation().store.clone();
    db.create_resource(
...
async fn test_foreground_delete_mark_retries_internal_rv_conflict_without_user_rv_precondition() {
    ...
    let bumped = db
        .update_status_only_with_preconditions(
            ...
            klights_cluster_core::ResourcePreconditions::from_resource(&stale),
        )
        .await
        .unwrap();
    ...
    let updated =
        crate::bootstrap::native_api_composition::support::mark_foreground_deletion_with_retry(
            &db,
            "v1",
            "ReplicationController",
            Some("default"),
            "gc-foreground-race",
            stale,
            klights_cluster_core::ResourcePreconditions::uid("rc-foreground-race-uid"),
        )
        .await
...
async fn test_finalizer_delete_mark_retries_internal_rv_conflict_preserving_concurrent_update() {
    ...
    let outcome = crate::bootstrap::native_api_composition::support::complete_non_foreground_delete_with_live_recheck(
        &db,
        crate::bootstrap::native_api_composition::support::GeneratedDeleteCompletionRequest {
            target: k8s_native_service::generic_command::ResourceDeleteTarget {
                api_version: "v1",
            ...
            initial_resource: stale,
            delete_preconditions: klights_cluster_core::ResourcePreconditions::uid(
    ...
    let updated = match outcome {
        crate::bootstrap::native_api_composition::support::DeleteCompletion::MarkedTerminating(
            resource,
        ) => resource,
...
async fn test_pod_and_service_proxy_filters_sensitive_forwarded_headers() {
    ...
    let db = state.resource_mutation().store.clone();
    let bootstrap_token =
        crate::bootstrap::native_api_composition::support::create_worker_bootstrap_token(&db)
            .await
            .unwrap();
    let app = state.router();
...
async fn namespace_delete_returns_while_picked_up_pods_wait_for_actor_finalization() {
    ...
    // Subscribe before finalization so an asynchronously accepted Raft delete
    // cannot commit between the actor-owned call and the event barrier.
    let mut pod_watch = state.subscribe_watch("v1", "Pod");
    ...
    for (name, uid) in &pod_uids {
        ...
        assert!(
            accepted,
            "actor-owned finalization must remove or accept the UID-qualified delete for {name}"
        );
    }

    // Accepted is intentionally asynchronous. Wait on committed DELETED
    // events rather than assuming acceptance means the datastore row is gone.
    let mut pending_uids = pod_uids
        .iter()
        .map(|(_name, uid)| uid.as_str())
        .collect::<std::collections::HashSet<_>>();
    tokio::time::timeout(std::time::Duration::from_secs(10), async {
        while !pending_uids.is_empty() {
            let event = pod_watch
                .recv()
                .await
                .expect("Pod delete watch must remain live through actor finalization");
            if event.event_type != klights_watch::EventType::Deleted {
                continue;
            }
            let namespace = event
                .object
    ...
    .await
    .unwrap_or_else(|_| {
        panic!("committed Pod delete events did not arrive for UIDs {pending_uids:?}")
    });

    for (name, _uid) in &pod_uids {
        assert!(
            db.get_resource("v1", "Pod", Some("gc-cleanup"), name)
                .await
                .unwrap()
                .is_none(),
            "committed DELETED event must be a post-commit barrier for {name}"
        );
    }
