...
async fn test_metrics_k8s_io_lists_and_gets_metrics_for_existing_nodes_and_pods() {
    ...
    let state = build_test_app_state().await;
    state.set_node_metrics(std::sync::Arc::new(StaticNodeMetrics { result }));
    let db = state.resource_mutation().store.clone();
    let app = state.router();
    db.create_resource(
        "v1",
...
async fn test_aggregated_discovery_includes_crd_synced_from_datastore() {
    ...
    .await
    .unwrap();
    state.sync_crd_registry_from_datastore().await.unwrap();
...
async fn test_service_status_subresource_preserves_fields_and_negotiates_json_protobuf() {
    ...
    async fn decode_response(
        ...
        if expect_protobuf {
            ...
            k8s_native_service::test_protobuf::decode_protobuf(&body[4..]).unwrap()
        } else {
            assert!(
    ...
    for case in cases {
        let body = if case.request_content_type.contains("protobuf") {
            Body::from(
                k8s_native_service::test_protobuf::encode_protobuf(&case.request_body).unwrap(),
            )
        } else {
...
async fn test_deployment_status_put_repairs_empty_type_meta_in_response() {
    ...
    // The deterministic controller drain has completed the queued reconcile,
    // so the next read must already contain the repaired conditions.
    let req = Request::builder()
        .method("GET")
...
async fn test_deployment_unconditional_main_update_ignores_status_churn() {
    ...
    let pause = state.install_resource_mutation_pause(
        crate::bootstrap::native_api_composition::support::IntegrationResourceMutationPauseOperation::MainUpdate,
        "apps/v1",
        "Deployment",
        Some("deploy-main-update-race"),
        "web",
    ...
    db.update_status_only_with_preconditions(
        ...
        klights_cluster_core::ResourcePreconditions::uid(&created.uid),
    )
    .await
    .unwrap();
    pause.resume();
