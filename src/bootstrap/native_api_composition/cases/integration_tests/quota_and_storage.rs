...
/// P12.2b-local persistence adapter for the P12.2c PVC algorithm tests.
/// It is deliberately private to this consumer and exposes only PVC/status
/// operations over the canonical same-router resource fixture.
#[derive(Clone)]
struct PvcResourceFixtureStore(klights_cluster_datastore::test_support::ResourceTestStore);

fn pvc_store_error(error: anyhow::Error) -> klights_reconcile_api::ControllerStoreError {
...
impl klights_controllers::common::ControllerStatusStore for PvcResourceFixtureStore {
...
#[async_trait::async_trait]
impl klights_controllers::pvc::PvcStore for PvcResourceFixtureStore {
    async fn get_pvc(
        &self,
        namespace: &str,
        name: &str,
    ...
    async fn create_persistent_volume(
        ...
        self.0
            .create_resource("v1", "PersistentVolume", None, name, value)
            .await
            .map_err(pvc_store_error)
    }
...
async fn test_csistoragecapacity_list_protobuf_decodes_as_native_list() {
    ...
    let unknown = k8s_native_service::test_protobuf::Unknown::decode(&body[4..]).unwrap();
    assert!(
...
async fn test_patch_pvc_status_condition_survives_rejected_stale_controller_commit() {
    ...
    assert!(result.terminal_rejection().is_some());
    assert_eq!(result.applied_resource_version(), None);
...
async fn test_pvc_binding_result_is_same_for_json_and_protobuf_creation() {
    ...
    let local_path_provisioner_root = std::env::temp_dir()
        .join("klights-api-tests")
        .join("pvc-bind-parity")
        .join("local-path-provisioner");
    let pvc_store = PvcResourceFixtureStore(db.clone());
    let _ = klights_controllers::pvc::reconcile_pvc(
        &crate::bootstrap::native_api_composition::support::file_process_executor(),
...
async fn test_runtimeclass_admission_rejects_protobuf_pod_with_missing_runtimeclass() {
    ...
    let pod_pb = k8s_native_service::test_protobuf::encode_protobuf(&pod_json).unwrap();
    let req = Request::builder()
        .method("POST")
...
async fn test_events_v1_create_is_listed_via_core_events_source_selector() {
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let listed = k8s_native_service::test_protobuf::decode_protobuf(&bytes[4..]).unwrap();
...
#[tokio::test]
async fn events_v1_cluster_wide_protobuf_list_encodes_conformance_event() {
    use axum::{
        body::{Body, to_bytes},
        http::{Request, StatusCode},
    };
    ...
    let app = build_test_router().await;
    let ns = format!("events-conformance-{}", uuid::Uuid::new_v4().simple());
    ...
    // Existing core/v1 Event rows are intentionally visible through the
    // events.k8s.io/v1 compatibility read. This is the legacy shape observed
    // in the conformance cluster alongside the native events/v1 row.
    let legacy_event = json!({
        "apiVersion": "v1",
        "kind": "Event",
        "metadata": {"name": "legacy-event", "namespace": ns},
        "involvedObject": {"namespace": ns, "kind": "Pod", "name": "legacy-pod"},
        "message": "Successfully pulled image",
        "reason": "Pulled",
        "source": {"component": "kubelet", "host": "test-node"},
        "firstTimestamp": "2017-09-19T13:49:10Z",
        "lastTimestamp": "2017-09-19T13:49:11Z",
        "count": 1,
        "type": "Normal"
    });
    let req = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(serde_json::to_vec(&legacy_event).unwrap()))
        .unwrap();
    let resp = app.clone().oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::CREATED);

    // Exact field shape authored by the Kubernetes v1.34 Events API
    // conformance test. The namespace is supplied by the namespaced route.
    ...
    let mut raw = Vec::new();
    k8s_native_service::test_protobuf::Message::encode(&event, &mut raw).unwrap();
    let envelope = k8s_native_service::test_protobuf::Unknown {
        type_meta: Some(k8s_native_service::test_protobuf::TypeMeta {
    ...
    k8s_native_service::test_protobuf::Message::encode(&envelope, &mut encoded_event).unwrap();
    let req = Request::builder()
        .method("POST")
        .uri(format!("/apis/events.k8s.io/v1/namespaces/{ns}/events"))
        .header("content-type", "application/vnd.kubernetes.protobuf")
    ...
    let uri = "/apis/events.k8s.io/v1/events";
    let req = Request::builder()
        .method("GET")
        .uri(uri)
    ...
    assert_eq!(
        resp.status(),
        StatusCode::OK,
        "cluster-wide events.k8s.io/v1 EventList must encode as protobuf"
    );
    let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let protobuf_list = k8s_native_service::test_protobuf::decode_protobuf(&bytes[4..]).unwrap();

    for list in [&json_list, &protobuf_list] {
        let items = list["items"].as_array().expect("items must be an array");
        assert_eq!(items.len(), 2, "unexpected EventList: {list}");

        let conformance = items
            .iter()
            .find(|item| item["metadata"]["name"] == "event-test")
            .expect("conformance Event must be listed");
        assert_eq!(conformance["apiVersion"], "events.k8s.io/v1");
        assert_eq!(conformance["metadata"]["namespace"], ns);
        assert_eq!(conformance["eventTime"], "2017-09-19T13:49:16.000000Z");
        assert_eq!(conformance["reportingController"], "test-controller");
        assert_eq!(conformance["reportingInstance"], "test-node");
        assert_eq!(conformance["action"], "Do");
        assert_eq!(conformance["reason"], "Test");
        assert_eq!(conformance["note"], "This is event-test");
        assert_eq!(conformance["type"], "Normal");

        let legacy = items
            .iter()
            .find(|item| item["metadata"]["name"] == "legacy-event")
            .expect("legacy core/v1 Event must be converted and listed");
        assert_eq!(legacy["apiVersion"], "events.k8s.io/v1");
        assert_eq!(legacy["kind"], "Event");
        assert_eq!(legacy["metadata"]["namespace"], ns);
        assert_eq!(legacy["regarding"]["name"], "legacy-pod");
        assert_eq!(legacy["note"], "Successfully pulled image");
        assert_eq!(legacy["deprecatedSource"]["component"], "kubelet");
        assert_eq!(legacy["deprecatedSource"]["host"], "test-node");
        assert_eq!(legacy["deprecatedFirstTimestamp"], "2017-09-19T13:49:10Z");
        assert_eq!(legacy["deprecatedLastTimestamp"], "2017-09-19T13:49:11Z");
        assert_eq!(legacy["deprecatedCount"], 1);
        assert_eq!(legacy["reason"], "Pulled");
        assert_eq!(legacy["type"], "Normal");
    }
}
...
async fn test_core_event_update_protobuf_preserves_series() {
    ...
    let updated_pb = k8s_native_service::test_protobuf::encode_protobuf(&updated).unwrap();
    let req = Request::builder()
        .method("PUT")
...
async fn test_ingress_status_update_with_protobuf_preserves_load_balancer() {
    ...
    let status_update_pb =
        k8s_native_service::test_protobuf::encode_protobuf(&status_update).unwrap();
    let req = Request::builder()
        .method("PUT")
...
async fn test_mutating_webhook_pod_create_applies_init_container_defaults_after_mutation_protobuf()
{
    ...
    let pod: serde_json::Value = if content_type.contains("application/vnd.kubernetes.protobuf") {
        k8s_native_service::test_protobuf::decode_protobuf(&response_bytes).unwrap()
    } else {
...
async fn test_csr_status_subresource_get_put() {
    ...
    let db = crate::bootstrap::native_api_composition::support::in_memory().await;
...
async fn test_ephemeral_containers_patch_visible_on_pod_get_for_followup_put() {
    ...
    let bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let mut pod = k8s_native_service::test_protobuf::decode_protobuf(&bytes).unwrap();
...
/// HTTP-level safety net proving POST /api/v1/.../pods runs native
/// ServiceAccount projection defaulting before persistence.
#[tokio::test]
