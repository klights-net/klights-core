...
/// Private P12.2b persistence projection for this file's retained namespace
/// bootstrap algorithm coverage. The controller remains owned by P12.2c; this
/// adapter only maps its exact storage contract to the canonical fixture.
#[derive(Clone)]
struct NamespaceResourceFixtureStore(klights_cluster_datastore::test_support::ResourceTestStore);

fn namespace_store_error(error: anyhow::Error) -> klights_reconcile_api::ControllerStoreError {
...
impl klights_controllers::namespace::NamespaceBootstrapStore for NamespaceResourceFixtureStore {
    ...
    async fn create_default_service_account(
        ...
        self.0
            .create_resource("v1", "ServiceAccount", Some(namespace), "default", value)
            .await
            .map_err(namespace_store_error)
    }
    ...
    async fn create_configmap(
        ...
        self.0
            .create_resource("v1", "ConfigMap", Some(namespace), name, value)
            .await
            .map_err(namespace_store_error)
    }
    ...
    async fn update_configmap(
        ...
        self.0
            .update_resource_with_preconditions(
                ...
                name,
                value,
                klights_cluster_core::ResourcePreconditions::resource_version(
...
async fn test_create_statefulset_initializes_revision_status() {
    ...
    let app = crate::bootstrap::native_api_composition::support::build_test_app_state_with_operational_endpoints()
        .await
        .router();
...
async fn test_create_namespace_auto_creates_default_service_account() {
    ...
    let store = NamespaceResourceFixtureStore(db.clone());
    ...
    klights_controllers::namespace::create_default_service_account_at(
        ...
        crate::bootstrap::native_api_composition::support::deterministic_controller_identity()
            .as_ref(),
    )
    .await
    .unwrap();
...
async fn api_put_get_networkpolicy_protobuf_roundtrip() {
    ...
    let np_pb = k8s_native_service::test_protobuf::encode_protobuf(&np_json).unwrap();
    let create_resp = app
        .clone()
        .oneshot(
            Request::builder()
    ...
    let decoded = k8s_native_service::test_protobuf::decode_protobuf(&body[4..]).unwrap();
