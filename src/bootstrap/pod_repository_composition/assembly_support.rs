...
//! leader/worker composition (`crate::bootstrap::pod_repository_composition::*`,
//! `crate::bootstrap::composition_adapters::*`, `crate::bootstrap::node_store`,
//! `crate::bootstrap::authority`) the same way production bootstrap does.
...
//! `src/bootstrap/pod_repository_composition/workqueue_tests.rs` precedent.
...
pub(crate) mod support {
    ...
    -> anyhow::Result<WorkerFinalizationDeliveryOutcome> {
        let sqlite = crate::bootstrap::cluster_store::selector::canonical_sqlite_fixture().await?;
    ...
    impl PodRepositoryScenarioOwner {
        ...
        async fn new_exact_on(
            ...
            let sqlite = match sqlite {
                Some(sqlite) => sqlite,
                None => crate::bootstrap::cluster_store::selector::canonical_sqlite_fixture()
                    .await
