...
pub enum IntegrationGcFixtureKind {
    Pod,
    Service,
    Deployment,
    ReplicaSet,
...
impl IntegrationGcFixtureKind {
    const fn api_version(self) -> &'static str {
        match self {
            ...
            | Self::ConfigMap
            | Self::Secret => "v1",
            Self::Deployment | Self::ReplicaSet => "apps/v1",
            Self::CustomResourceDefinition => "apiextensions.k8s.io/v1",
            Self::Widget => "example.com/v1",
            Self::ClusterRoleBinding => "rbac.authorization.k8s.io/v1",
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IntegrationGcNonPodKind {
    Service,
    Deployment,
    ReplicaSet,
    ReplicationController,
...
impl From<IntegrationGcNonPodKind> for IntegrationGcFixtureKind {
    fn from(value: IntegrationGcNonPodKind) -> Self {
        match value {
            IntegrationGcNonPodKind::Service => Self::Service,
            IntegrationGcNonPodKind::Deployment => Self::Deployment,
            IntegrationGcNonPodKind::ReplicaSet => Self::ReplicaSet,
            IntegrationGcNonPodKind::ReplicationController => Self::ReplicationController,
            IntegrationGcNonPodKind::ConfigMap => Self::ConfigMap,
            IntegrationGcNonPodKind::Secret => Self::Secret,
            IntegrationGcNonPodKind::CustomResourceDefinition => Self::CustomResourceDefinition,
            IntegrationGcNonPodKind::Widget => Self::Widget,
            IntegrationGcNonPodKind::ClusterRoleBinding => Self::ClusterRoleBinding,
        }
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IntegrationGcOwnerIdentity {
    kind: Option<IntegrationGcFixtureKind>,
    namespace: Option<String>,
    name: String,
    uid: String,
}

pub struct IntegrationGcStalePodSnapshot {
    resource: klights_cluster_core::Resource,
}

pub struct IntegrationGcUnscheduledPodObservation {
    identity: klights_types::PodIdentity,
    observed_resource_version: i64,
}

pub struct IntegrationGcActorSink {
    repository: Arc<crate::kubelet::pod_repository::PodRepository>,
    node_local: Arc<crate::datastore::node_local::NodeLocalStores>,
}

impl klights_reconcile_api::GcPodDeleteSink for IntegrationGcActorSink {
...
impl IntegrationGcActorSink {
    pub async fn claim_queued_uid(&self) -> anyhow::Result<Option<String>> {
        let row = self
            .node_local
            .pod_workqueue()
        ...
        Ok(row.and_then(|row| match row.identity() {
            klights_node_store::PodWorkIdentity::Pod(identity) => Some(identity.uid.clone()),
            klights_node_store::PodWorkIdentity::Namespace { .. } => None,
        }))
    }

    pub async fn finalize_pod_after_actor_cleanup(
        &self,
        namespace: &str,
        name: &str,
        uid: &str,
...
impl IntegrationGcOwnerIdentity {
    pub fn new(
        kind: IntegrationGcFixtureKind,
    ...
    pub fn empty(namespace: Option<&str>) -> Self {
        Self {
            kind: None,
            namespace: namespace.map(str::to_owned),
            name: String::new(),
    ...
    fn api_version(&self) -> &'static str {
        self.kind.map_or("", IntegrationGcFixtureKind::api_version)
    }

    fn kind_name(&self) -> &'static str {
        self.kind.map_or("", IntegrationGcFixtureKind::kind)
    }
}
...
struct IntegrationStatusChurningGcStore {
...
impl IntegrationStatusChurningGcStore {
...
impl GcResourceStore for IntegrationStatusChurningGcStore {
...
/// Focused root-composition fixture for Kubernetes garbage-collection tests.
pub struct IntegrationGcCompositionHarness {
    db: crate::datastore::sqlite::Datastore,
    coordination: klights_controllers::ControllerCoordination,
}

impl IntegrationGcCompositionHarness {
    pub async fn new() -> anyhow::Result<Self> {
        Ok(Self {
            db: crate::datastore::sqlite::Datastore::new_in_memory().await?,
            coordination: klights_controllers::ControllerCoordination::new(),
        })
    }
    ...
    pub async fn actor_sink(&self) -> IntegrationGcActorSink {
        let composition = IntegrationPodRepositoryComposition::new_gc_on(self.db.clone()).await;
        IntegrationGcActorSink {
            repository: composition.repository,
            node_local: composition.node_local.expect("GC actor sink workqueue"),
        }
    }
    ...
    pub async fn observe(
        ...
        use crate::datastore::DatastoreBackend as _;
        self.db
            .get_resource(kind.api_version(), kind.kind(), namespace, name)
            .await
    }
    ...
    pub async fn remove_non_pod(
        &self,
        kind: IntegrationGcNonPodKind,
        namespace: Option<&str>,
        name: &str,
    ) -> anyhow::Result<()> {
        use crate::datastore::DatastoreBackend as _;
        let kind = IntegrationGcFixtureKind::from(kind);
        self.db
            .delete_resource(kind.api_version(), kind.kind(), namespace, name)
            .await
    }

    fn store(&self) -> impl klights_controllers::gc::GcResourceStore + '_ {
        crate::bootstrap::controller_adapters::controller_runtime_adapter::controller_store_for_test(
            &self.db,
        )
    }
    ...
    ) -> anyhow::Result<klights_controllers::gc::OwnerReferenceReconcile> {
    ...
    pub fn stale_pod_snapshot(
        &self,
        namespace: &str,
        name: &str,
        uid: &str,
        missing_owner: &IntegrationGcOwnerIdentity,
    ) -> IntegrationGcStalePodSnapshot {
        IntegrationGcStalePodSnapshot {
            resource: klights_cluster_core::Resource {
                id: 0,
                ...
                data: Arc::new(serde_json::json!({
                    ...
                    "metadata": {
                        ...
                        "uid": uid,
                        "ownerReferences": [{
                            "apiVersion": missing_owner.api_version(),
                            "kind": missing_owner.kind_name(),
                            "name": missing_owner.name,
                            "uid": missing_owner.uid
                        }]
                    },
                    "spec": {"nodeName": "gc-worker"}
                })),
                resource_version: 1,
            },
        }
    ...
    pub async fn reconcile_stale_pod_snapshot(
        &self,
        snapshot: IntegrationGcStalePodSnapshot,
        pod_delete_sink: &dyn klights_reconcile_api::GcPodDeleteSink,
    ) -> anyhow::Result<klights_controllers::gc::OwnerReferenceReconcile> {
        self.reconcile_owner_references(snapshot.resource, pod_delete_sink)
            .await
    }

    pub async fn cascade_delete(
        &self,
        owner: &IntegrationGcOwnerIdentity,
        pod_delete_sink: &dyn klights_reconcile_api::GcPodDeleteSink,
    ) -> anyhow::Result<()> {
    ...
    pub async fn cascade_sweep_once(
        &self,
        owner: &IntegrationGcOwnerIdentity,
        pod_delete_sink: &dyn klights_reconcile_api::GcPodDeleteSink,
    ) -> anyhow::Result<bool> {
    ...
    pub async fn finalize_foreground_owner_if_ready(
        &self,
        owner: &klights_cluster_core::Resource,
        pod_delete_sink: &dyn klights_reconcile_api::GcPodDeleteSink,
    ) -> anyhow::Result<bool> {
    ...
    pub async fn orphan_children(&self, owner: &IntegrationGcOwnerIdentity) -> anyhow::Result<()> {
    ...
    /// Core-internal scenario for owner-reference retries racing status-only writes.
    pub async fn run_orphan_status_churn(
        &self,
        owner: &IntegrationGcOwnerIdentity,
        typed_conflicts: usize,
        fail_main_update: bool,
    ) -> anyhow::Result<usize> {
        let store = IntegrationStatusChurningGcStore {
            db: self.db.clone(),
            update_attempts: AtomicUsize::new(0),
            typed_conflicts_remaining: AtomicUsize::new(typed_conflicts),
            fail_main_update,
        };
        klights_controllers::gc::orphan_children(
            &store,
        ...
        .await?;
        Ok(store.update_attempts.load(Ordering::SeqCst))
    }

    pub async fn observe_terminating_unscheduled_pod(
        &self,
        namespace: &str,
        name: &str,
        uid: &str,
    ) -> anyhow::Result<Option<IntegrationGcUnscheduledPodObservation>> {
        let Some(resource) = self
            .observe(IntegrationGcFixtureKind::Pod, Some(namespace), name)
            .await?
        else {
            return Ok(None);
        };
        let node_name = resource
            ...
            .unwrap_or_default();
        let deletion_timestamp = resource
            .data
            .pointer("/metadata/deletionTimestamp")
            .and_then(serde_json::Value::as_str)
            .unwrap_or_default();
        let finalizers_pending = resource
            .data
        ...
        if resource.uid != uid
            || !node_name.is_empty()
            || deletion_timestamp.is_empty()
            || finalizers_pending
        {
            return Ok(None);
        }
        Ok(Some(IntegrationGcUnscheduledPodObservation {
            identity: klights_types::PodIdentity::new(namespace, name, uid),
            observed_resource_version: resource.resource_version,
        }))
    }

    pub async fn finalize_unscheduled_pod_with_observed_cas(
        &self,
        observation: IntegrationGcUnscheduledPodObservation,
    ) -> anyhow::Result<bool> {
        let store = Arc::new(crate::pod_repository_composition::new_pod_store(Arc::new(
            self.db.clone(),
        )));
        let deletion =
        ...
        Ok(matches!(
            outcome,
            klights_pod_api::UnscheduledPodDeletionOutcome::Removed
        ))
    }
}
...
impl IntegrationPodRepositoryComposition {
    async fn new_gc_on(sqlite: crate::datastore::sqlite::Datastore) -> Self {
        Self::new_exact_on(
            Some(sqlite),
            None,
            false,
...
pub(crate) mod node_delivery_support {
    ...
    impl IntegrationNodeDeliveryCluster {
        ...
        pub fn node_ports(&self, authenticated_node: &str) -> super::IntegrationLeaderRpcNodePorts {
            super::IntegrationLeaderRpcComposition::local_node_ports(
                self.db.clone(),
        ...
        pub fn heartbeat_event_source(
            ...
            let passive =
                super::IntegrationLeaderRpcComposition::passive_reads_for(self.db.as_ref());
            super::IntegrationLeaderRpcComposition::node_heartbeat_event_source(
                &passive,
                self.db.clone(),
            )
        }
