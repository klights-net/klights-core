...
async fn test_job_single_completion() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-1")
        .await
        .unwrap();
...
async fn test_job_multiple_completions() {
    ...
    let _job = reconcile_job_test_with_identity(&db, &job, "test-node", identity.as_ref())
        .await
        .unwrap();
    let mut pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-2")
        .await
        .unwrap();
    ...
    let _job = reconcile_job_test_with_identity(&db, &job, "test-node", identity.as_ref())
        .await
        .unwrap();
    pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-2")
        .await
        .unwrap();
...
async fn test_job_parallelism() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-3")
        .await
        .unwrap();
...
async fn test_job_backoff_limit() {
    ...
    let _job = reconcile_job_test_with_identity(&db, &job, "test-node", identity.as_ref())
        .await
        .unwrap();
    let mut pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-4")
        .await
        .unwrap();
    ...
    let _job = reconcile_job_test_with_identity(&db, &job, "test-node", identity.as_ref())
        .await
        .unwrap();
    pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-4")
        .await
        .unwrap();
    assert_eq!(pods.len(), 2, "Should create second pod");
...
async fn test_job_status_complete() {
    ...
    let _job = reconcile_job_test(&db, &job, "test-node").await.unwrap();
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-5")
        .await
        .unwrap();
...
async fn test_job_status_failed() {
    ...
    let _job = reconcile_job_test(&db, &job, "test-node").await.unwrap();
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-6")
        .await
        .unwrap();
...
async fn test_job_pod_without_phase_counted_as_active() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-nophase")
        .await
        .unwrap();
...
async fn test_job_does_not_exceed_parallelism() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-limit")
        .await
        .unwrap();
    assert_eq!(pods.len(), 2, "Should create 2 pods for parallelism=2");
...
async fn test_job_pod_has_owner_references() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-owner-uid")
        .await
        .unwrap();
    assert_eq!(pods.len(), 1);
...
async fn test_job_complete_does_not_create_more_pods() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-done")
        .await
        .unwrap();
    let mut pod_data: serde_json::Value = (*pods[0].data).clone();
    pod_data["status"] = json!({"phase": "Succeeded"});
...
async fn test_job_success_policy_succeeded_count_sets_success_criteria_met() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-sp-1")
        .await
        .unwrap();
...
async fn test_job_success_policy_succeeded_indexes_sets_success_criteria_met() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-sp-2")
        .await
        .unwrap();
...
async fn test_indexed_job_preserves_completed_indexes_after_terminal_pods_are_deleted() {
    ...
    let owned = crate::controller_test_debt::find_owned_pods(&db, "default", "job-indexed-history")
        .await
        .unwrap();
...
async fn test_indexed_job_does_not_complete_with_duplicate_succeeded_index() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-indexed-dup")
        .await
        .unwrap();
    assert!(
        pods.iter().any(|pod| {
