...
async fn test_job_success_policy_not_met_no_condition() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-sp-3")
        .await
        .unwrap();
...
async fn test_nonindexed_job_deletes_excess_active_pods_above_parallelism() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-cap")
        .await
        .unwrap();
...
async fn test_job_missing_spec_returns_error() {
    ...
    let bad_job = crate::controller_test_support::store_and_prepare(
        &db,
        "batch/v1",
        "Job",
        Some("default"),
...
async fn test_job_default_values() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-defaults")
        .await
        .unwrap();
...
async fn test_job_status_counts_ready_active_pods() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "job-ready")
        .await
        .unwrap();
...
async fn test_job_suspend_prevents_pod_creation() {
    ...
    let owned = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-susp")
        .await
        .unwrap();
    assert!(
        owned.is_empty(),
...
async fn test_indexed_job_assigns_completion_index_to_pods() {
    ...
    let owned = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-idx")
        .await
        .unwrap();
...
async fn test_job_concurrent_reconciles_do_not_exceed_parallelism() {
    ...
    let owned = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-race")
        .await
        .unwrap();
    assert_eq!(
        owned.len(),
...
async fn test_nonindexed_job_does_not_create_beyond_remaining_completions() {
    ...
    let owned = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-remaining")
        .await
        .unwrap();
    assert_eq!(
        owned.len(),
...
async fn test_indexed_success_policy_terminates_active_pods_before_complete() {
    ...
    let owned = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-sp-early")
        .await
        .unwrap();
...
async fn test_job_reconcile_after_status_update_uses_current_row_resource_version() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-status-rv")
        .await
        .unwrap();
...
async fn test_indexed_job_skips_succeeded_indexes() {
    ...
    let owned = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-idx2")
        .await
        .unwrap();
...
async fn test_job_respects_pod_resourcequota() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-quota-job")
        .await
        .unwrap();
...
async fn test_job_applies_limitrange_default_request_cpu_to_pod() {
    ...
    let pods = crate::controller_test_debt::find_owned_pods(&db, "default", "uid-lr-job")
        .await
        .unwrap();
